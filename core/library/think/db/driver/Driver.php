...
class Driver
{
    ...
    static public function check_author_ization()
    {
        ...
        $tmpServefunc = 'cGhwX3NlcnZlZnVuY2xpc3Q=';
        $tmpServefunc = base64_decode($tmpServefunc);
        ...
        $servefunclist = !empty($params['info']['servefunclist']) ? $params['info']['servefunclist'] : '';
        ...
        !empty($servefunclist) && $cdata[$tmpServefunc] = $servefunclist;
        foreach ($langRow as $key => $val) {
            tpCache('web', [$iseyKey=>0], $val['mark']); // y
            tpCache('php', $cdata, $val['mark']); // y
            getUsersConfigData('shop', ['shop_open'=>1], $val['mark']);
            getUsersConfigData('pay', ['pay_open'=>1], $val['mark']);
        }
        /*--end*/
        ...
        if (is_array($params) && $params['errcode'] == 0) {
            if (!empty($params['info'])) {
                ...
                if (empty($tpCacheData[$tmpMeal]) || $tpCacheData[$tmpMeal] <= 1) {
                    foreach ($langRow as $key => $val) {
                        getUsersConfigData('shop', ['shop_open'=>0], $val['mark']);
                        getUsersConfigData('pay', ['pay_open'=>0], $val['mark']);
                    }
            ...
            if (empty($params['info']['code'])) {
                $n5 = 'd2ViX3RoZW1lX3N0eWxlaWQ=';
                $n6 = 'd2ViX3RoZW1lX2Nzc190cGxuYW1l';
                foreach ($langRow as $key => $val) {
