        ...
        if (3 == $seo_pseudo) { // 伪静态模式 by 小虎哥
            if (!isset($this->options['query'][$url_screen_var])) { // 不是筛选URL
                ...
                if (!empty($typeid)) {
                    ...
                    if (!in_array(MODULE_NAME.'@'.CONTROLLER_NAME, ['home@Search'])) {
                        if (preg_match('/\.html$/i', $url)) {
                            if (1 >= $this->currentPage) {
                                1 < $page && $url = preg_replace('/\.html$/i', "/list_all_{$page}.html", $url);
                            } else {
                                $url = preg_replace('/\/list_all_(\d+)\.html$/i', '.html', $url);
                                1 < $page && $url = preg_replace('/\.html$/i', "/list_all_{$page}.html", $url);
                            }
                        } else {
                            $url .= '/';
                            if (1 >= $this->currentPage) {
                                1 < $page && $url .= "list_all_{$page}/";
                            } else {
                                $url = preg_replace('/\/list_all_(\d+)\/$/i', '/', $url);
                                1 < $page && $url .= "list_all_{$page}/";
                            }
                        }
                        unset($parameters[$this->options['var_page']]);
                        if (isset($parameters['typeid_tmp'])) {
                            unset($parameters['typeid_tmp']);
                        }
                    }
                }
            }
