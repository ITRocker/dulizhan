...
class Zan extends Taglib
{
    ...
    protected $tags = [
        ...
        'type'       => ['attr' => 'modelid,channelid,typeid,type,empty,dirname,id,addfields,addtable'],
        'single'     => ['attr' => 'typeid,aid,type,empty,id,addfields,empty'],
        ...
        'uitype'     => ['attr' => 'e-id,e-page,id,typeid,modelid,channelid'],
        'uiarclist'  => ['attr' => 'e-id,e-page,id,typeid,modelid,channelid', 'alias' => 'uiartlist'],
        'uichannel'  => ['attr' => 'e-id,e-page,id,typeid,modelid,channelid', 'alias' => 'uimodels'],
        'uimap'      => ['attr' => 'e-id,e-page,id,width,height'],
        'uicode'     => ['attr' => 'e-id,e-page,id'],
        'uibackground'=> ['attr' => 'e-id,e-page,id,e-img', 'close' => 0],
        'uisingle'   => ['attr' => 'e-id,e-page,id,typeid,aid'],
        'sql'        => ['attr' => 'sql,key,id,mod,cachetime,empty', 'close'=>1, 'level'=>3], // zan sql 万能标签
    ...
    public function tagSql($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSql = new \think\template\taglib\zan\TagSql;';
    ...
    public function tagUi($tag)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagUi = new \think\template\taglib\zan\TagUi;';
    ...
    public function tagUiupload($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUiupload = new \think\template\taglib\zan\TagUiupload;';
    ...
    public function tagUichannel($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUichannel = new \think\template\taglib\zan\TagUichannel;';
        $parseStr .= ' $__LIST__ = $tagUichannel->getUichannel("'.$modelid.'","'.$typeid.'","'.$e_id.'", "'.$e_page.'"); ?>';
        ...
        $parseStr .= ' $ui_modelid = !empty($'.$id.'["info"]["modelid"]) ? $'.$id.'["info"]["modelid"] : "";';
        // $parseStr .= ' $ui_row = !empty($'.$id.'["info"]["row"]) ? $'.$id.'["info"]["row"] : "";';
        $parseStr .= '?>';
        $parseStr .= $content;
        $parseStr .= '<?php $ui_typeid = $ui_modelid = $ui_row = ""; ?>';
        $parseStr .= '<?php endif; ?>';
    ...
    public function tagUitype($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUitype = new \think\template\taglib\zan\TagUitype;';
        $parseStr .= ' $__LIST__ = $tagUitype->getUitype("'.$modelid.'","'.$typeid.'","'.$e_id.'", "'.$e_page.'"); ?>';
        ...
        $parseStr .= '?>';
        $parseStr .= $content;
        $parseStr .= '<?php $ui_typeid = $ui_modelid = ""; ?>';
        $parseStr .= '<?php endif; ?>';
    ...
    public function tagUiarclist($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUiarclist = new \think\template\taglib\zan\TagUiarclist;';
        $parseStr .= ' $__LIST__ = $tagUiarclist->getUiarclist("'.$modelid.'","'.$typeid.'","'.$e_id.'", "'.$e_page.'"); ?>';
    ...
    public function tagUihtml($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUihtml = new \think\template\taglib\zan\TagUihtml;';
    ...
    public function tagUimap($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUimap = new \think\template\taglib\zan\TagUimap;';
    ...
    public function tagUicode($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUicode = new \think\template\taglib\zan\TagUicode;';
    ...
    public function tagUitext($tag, $content)
    {
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUitext = new \think\template\taglib\zan\TagUitext;';
    ...
    public function tagUibackground($tag, $content)
    {
        ...
        $parseStr = ' <?php ';
        $parseStr .= ' $tagUibackground = new \think\template\taglib\zan\TagUibackground;';
     ...
     * 美化标签-单页编辑
     */
    public function tagUisingle($tag, $content)
    {
        $typeid = isset($tag['typeid']) ? $tag['typeid'] : '';
        ...
        $parseStr = '';
        $parseStr .= ' <?php ';
        $parseStr .= ' $tagUisingle = new \think\template\taglib\zan\TagUisingle;';
        $parseStr .= ' $__LIST__ = $tagUisingle->getUisingle("'.$typeid.'","'.$aid.'","'.$e_id.'", "'.$e_page.'"); ?>';

        $parseStr .= '<?php if((is_array($__LIST__)) && (!empty($__LIST__["info"]) || (($__LIST__["info"] instanceof \think\Collection || $__LIST__["info"] instanceof \think\Paginator ) && $__LIST__["info"]->isEmpty()))): ?>';
        $parseStr .= '<?php $'.$id.' = $__LIST__; ';
        $parseStr .= ' $ui_typeid = !empty($'.$id.'["info"]["typeid"]) ? $'.$id.'["info"]["typeid"] : "";';
        $parseStr .= ' $ui_aid = !empty($'.$id.'["info"]["aid"]) ? $'.$id.'["info"]["aid"] : "";';
        $parseStr .= '?>';
        $parseStr .= $content;
        $parseStr .= '<?php $ui_typeid = $ui_aid = ""; ?>';
        $parseStr .= '<?php endif; ?>';
    ...
    public function tagLoad($tag)
    {
        ...
        $parseStr .= $startStr;
        $parseStr .= ' <? $tagLoad = new \think\template\taglib\zan\TagLoad;';
    ...
    /**
     * static 标签解析 {zan:static file="/static/js/base.js" /}
     * 格式：{zan:static file="/static/css/base.css" /}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagStatic($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagStatic = new \think\template\taglib\zan\TagStatic;';
    ...
    /**
     * channel 标签解析 用于获取分类列表
     * 格式：type:son表示下级分类,self表示同级分类,top顶级分类
     * {zan:channel typeid='1' type='son' loop='10' empty='' name='' id='' key='' titlelen='' offset='' mod='' currentclass='active'}
    ...
    public function tagChannel($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagChannel = new \think\template\taglib\zan\TagChannel;';
    ...
    /**
     * channelartlist 标签解析 用于获取分类列表
     * 格式：type:son表示下级分类,self表示同级分类,top顶级分类
     * {zan:channelartlist typeid='1' type='son' loop='10'}
     *  <li><a href='{$field:typelink}'>{$field:typename}</a> </li> 
     * {/zan:channelartlist}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagChannelartlist($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagChannelartlist = new \think\template\taglib\zan\TagChannelartlist;';
     ...
     * 格式：
     * {zan:arclist modelid='1' typeid='1' loop='10' offset='0' titlelen='30' orderby ='aid desc' flag='' bodylen='160' empty='' id='field' mod='' name=''}
     *  {$field.title}
     *  {$field.typeid}
     * {/zan:arclist}
     * @access public
    ...
    public function tagArclist($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagArclist = new \think\template\taglib\zan\TagArclist;';
     ...
     * 格式：
     * {zan:list modelid='1' typeid='1' titlelen='30' orderby ='aid desc' flag='' bodylen='160' empty='' id='field' mod='' name=''}
     *  {$field.title}
     *  {$field.typeid}
     * {/zan:list}
     * @access public
    ...
    public function tagList($tag, $content)
    {
        ...
        $parseStr .= ' $tagList = new \think\template\taglib\zan\TagList;';
     ...
     * pagelist 标签解析
     * 在模板中获取列表的分页
     * 格式： {zan:pagelist listitem='info,index,end,pre,next,pageno' listsize='2'/}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagPagelist($tag)
    {
        ...
        $parseStr .= ' $tagPagelist = new \think\template\taglib\zan\TagPagelist;';
     ...
     * 格式： {zan:arcpagelist tagid='' pagesize='2'} {/zan:arcpagelist}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagArcpagelist($tag, $content)
    {
        ...
        $parseStr .= ' $tagArcpagelist = new \think\template\taglib\zan\TagArcpagelist;';
    ...
    public function tagPosition($tag)
    {
        ...
        $parseStr .= ' $tagPosition = new \think\template\taglib\zan\TagPosition;';
    ...
    public function tagSearchurl($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSearchurl = new \think\template\taglib\zan\TagSearchurl;';
    ...
    public function tagSearchform($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSearchform = new \think\template\taglib\zan\TagSearchform;';
    ...
    public function tagType($tag, $content)
    {
        ...
        $parseStr .= ' if(empty($typeid) && isset($channelartlist["id"]) && !empty($channelartlist["id"])) : $typeid = intval($channelartlist["id"]); endif; ';
        /*--end*/
        $parseStr .= ' $tagType = new \think\template\taglib\zan\TagType;';
        $parseStr .= ' $_result = $tagType->getType($modelid, $typeid, "'.$type.'", '.$addfields.',"' . $empty . '");';
        $parseStr .= ' ?>';
    ...
    /**
     * single标签解析 指定的单个分类的链接
     * 格式：
     * {zan:single typeid='' empty=''}
    ...
    public function tagSingle($tag, $content)
    {
        ...
        /*typeid的优先级别从高到低：装修数据 -> 标签属性值 -> 外层标签channelartlist属性值*/
        $parseStr .= ' if(isset($ui_typeid) && !empty($ui_typeid)) : $typeid = $ui_typeid; else: $typeid = '.$typeid.'; endif;';
        $parseStr .= ' if(isset($ui_aid) && !empty($ui_aid)) : $aid = $ui_aid; else: $aid = '.$aid.'; endif;';
        $parseStr .= ' if(empty($typeid) && isset($channelartlist["id"]) && !empty($channelartlist["id"])) : $typeid = intval($channelartlist["id"]); endif; ';
        /*--end*/
        $parseStr .= ' $tagSingle = new \think\template\taglib\zan\TagSingle;';
        $parseStr .= ' $_result = $tagSingle->getSingle($typeid, $aid, "'.$type.'", '.$addfields.',"' . $empty . '");';
        $parseStr .= ' ?>';
    ...
    /**
     * arcview标签解析 指定的单个分类的链接
     * 格式：
     * {zan:arcview aid='' empty=''}
     *  <a href="{$field:arcurl}">{$field:title}</a>
     * {/zan:arcview}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagArcview($tag, $content)
    {
        ...
        $parseStr .= ' $tagArcview = new \think\template\taglib\zan\TagArcview;';
    ...
    public function tagTag($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagTag = new \think\template\taglib\zan\TagTag;';
     ...
     * 格式：
     * {zan:tagarclist typeid='1' loop='10' offset='0' titlelen='30' orderby ='aid desc' flag='' bodylen='160' empty='' id='field' mod='' name=''}
     *  {$field.title}
     *  {$field.typeid}
     * {/zan:tagarclist}
     * @access public
    ...
    public function tagTagarclist($tag, $content)
    {
        ...
        $parseStr .= ' );';
        $parseStr .= ' $tag = '.var_export($tag,true).';';
        $parseStr .= ' $tagTagarclist = new \think\template\taglib\zan\TagTagarclist;';
    ...
    public function tagFlink($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagFlink = new \think\template\taglib\zan\TagFlink;';
    ...
    /**
     * language 标签解析 TAG调用
     * {zan:language loop='1' type='default'}
    ...
    public function tagLanguage($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagLanguage = new \think\template\taglib\zan\TagLanguage;';
    ...
    public function tagLang($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagLang = new \think\template\taglib\zan\TagLang;';
    ...
    public function tagAd($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        // 声明变量
        $parseStr .= ' $tagAd = new \think\template\taglib\zan\TagAd;';
    ...
    public function tagAdv($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagAdv = new \think\template\taglib\zan\TagAdv;';
    ...
    public function tagPrenext($tag, $content)
    {
        ...
        if (1 == $style) {
            ...
            $parseStr = '<?php ';
            $parseStr .= ' $tagPrenext = new \think\template\taglib\zan\TagPrenext;';
     ...
     * {zan:guestbookform type='default'}
     * {$field.value}
     * {/zan:guestbookform}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagGuestbookform($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagGuestbookform = new \think\template\taglib\zan\TagGuestbookform;';
     ...
     * 格式： {zan:field name="typename|html_msubstr=###,0,2" /}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagField($tag)
    {
        ...
        if (!empty($name)) {
            ...
            // 查询数据库获取的数据集
            $parseStr .= ' $tagField = new \think\template\taglib\zan\TagField;';
     ...
     * empty标签解析
     * 如果某个变量为empty 则输出内容
     * 格式： {zan:empty name="" }content{/zan:empty}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
     ...
     * notempty 标签解析
     * 如果某个变量不为empty 则输出内容
     * 格式： {zan:notempty name="" }content{/zan:notempty}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
     ...
     * foreach标签解析 循环输出数据集
     * 格式：
     * {zan:foreach name="userList" id="user" key="key" index="i" mod="2" offset="3" length="5" empty=""}
     * {user.username}
     * {/zan:foreach}
     * @access public
     * @param array $tag 标签属性
     ...
     * if标签解析
     * 格式：
     * {zan:if condition=" $a eq 1"}
     * {zan:elseif condition="$a eq 2" /}
     * {zan:else /}
     * {/zan:if}
     ...
     * switch标签解析
     * 格式：
     * {zan:switch name="a.name"}
     * {zan:case value="1" break="false"}1{/case}
     * {zan:case value="2" }2{/case}
     * {zan:default /}other
     * {/zan:switch}
     * @access public
     * @param array $tag 标签属性
     ...
     * 格式： {zan:compare name="" type="eq" value="" }content{/zan:compare}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
     ...
     * volist标签解析 循环输出数据集
     * 格式：
     * {zan:volist name="userList" id="user" empty=""}
     * {user.username}
     * {user.email}
     * {/zan:volist}
     * @access public
    ...
    public function tagGlobal($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagGlobal = new \think\template\taglib\zan\TagGlobal;';
    ...
    public function tagArcclick($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagArcclick = new \think\template\taglib\zan\TagArcclick;';
    ...
    public function tagDowncount($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagDowncount = new \think\template\taglib\zan\TagDowncount;';
    ...
    public function tagCollectnum($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagCollectnum = new \think\template\taglib\zan\TagCollectnum;';
    ...
    public function tagFreebuynum($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagFreebuynum = new \think\template\taglib\zan\TagFreebuynum;';
     ...
     * weapp标签解析
     * 安装网站应用插件时自动在页面上追加代码
     * 格式： {zan:weapp type='default'}content{/zan:weapp}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
    ...
    public function tagWeapp($tag, $content)
    {
        ...
        $parseStr = ' <?php ';
        $parseStr .= ' $tagWeapp = new \think\template\taglib\zan\TagWeapp;';
     ...
     * 格式： {zan:range name="var|function"  value="val" type='in|notin' }content{/zan:range}
     * example: {zan:range name="a"  value="1,2,3" type='in' }content{/zan:range}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
     ...
     * present标签解析
     * 如果某个变量已经设置 则输出内容
     * 格式： {zan:present name="" }content{/zan:present}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
     ...
     * notpresent标签解析
     * 如果某个变量没有设置，则输出内容
     * 格式： {zan:notpresent name="" }content{/zan:notpresent}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
    ...
    /**
     * 判断是否已经定义了该常量
     * {zan:defined name='TXT'}已定义{/zan:defined}
     * @param array $tag
     ...
     * for标签解析
     * 格式：
     * {zan:for start="" end="" comparison="" step="" name=""}
     * content
     * {/zan:for}
     * @access public
     * @param array $tag 标签属性
    ...
    /**
     * url函数的tag标签
     * 格式：{zan:url link="模块/控制器/方法" vars="参数" suffix="true或者false 是否带有后缀" domain="true或者false 是否携带域名" /}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
     ...
     * 使用：
     * {zan:function name="func" vars="$data" call="$list" use="&$a,&$b"}
     *      {zan:if is_array($data)}
     *          {zan:foreach $data as $val}
     *              {~func($val) /}
     *          {/zan:foreach}
     *      {zan:else /}
     *          {$data}
     ...
     * 格式：
     * {zan:diyfield type="default" name="$zan.field.imgs" id="field"}
    ...
    public function tagDiyfield($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagDiyfield = new \think\template\taglib\zan\TagDiyfield;';
    ...
    public function tagVideoplay($tag, $content)
    {
        ...
        $parseStr .= ' $tagVideoplay = new \think\template\taglib\zan\TagVideoplay;';
    ...
    public function tagVideolist($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagVideolist = new \think\template\taglib\zan\TagVideolist;';
    ...
    public function tagAttribute($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagAttribute = new \think\template\taglib\zan\TagAttribute;';
    ...
    public function tagAttr($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagAttr = new \think\template\taglib\zan\TagAttr;';
    ...
    public function tagUser($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagUser = new \think\template\taglib\zan\TagUser;';
     ...
     * weapplist标签解析
     * 安装网站应用插件时自动在页面上追加代码
     * 格式： {zan:weapplist type='default'}content{/zan:weapplist}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
    ...
    public function tagWeapplist($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagWeapplist = new \think\template\taglib\zan\TagWeapplist;';
    ...
    public function tagUsermenu($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagUsermenu = new \think\template\taglib\zan\TagUsermenu;';
    ...
    public function tagSppurchase($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSppurchase = new \think\template\taglib\zan\TagSppurchase;';
    ...
    public function tagSpcart($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSpcart = new \think\template\taglib\zan\TagSpcart;';
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {zan:sporder loop='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/zan:sporder}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagSporder($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr = '<?php ';
        $parseStr .= ' $tagSporder = new \think\template\taglib\zan\TagSporder;';
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {zan:spsubmitorder loop='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/zan:spsubmitorder}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagSpsubmitorder($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr = '<?php ';
        $parseStr .= ' $tagSpsubmitorder = new \think\template\taglib\zan\TagSpsubmitorder;';
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {zan:sporderlist loop='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/zan:sporderlist}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagSporderlist($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSporderlist = new \think\template\taglib\zan\TagSporderlist;';
     ...
     * sppageorder 标签解析
     * 在模板中获取列表的分页
     * 格式： {zan:sppageorder listitem='info,index,end,pre,next,pageno' listsize='2'/}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {zan:spordergoods loop='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/zan:spordergoods}
     * @access public
     * @param array $tag 标签属性
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {zan:spaddress loop='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/zan:spaddress}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagSpaddress($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSpaddress = new \think\template\taglib\zan\TagSpaddress;';
    ...
    public function tagSpsearch($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSpsearch = new \think\template\taglib\zan\TagSpsearch;';
    ...
    public function tagSppayapilist($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSppayapilist = new \think\template\taglib\zan\TagSppayapilist;';
    ...
    public function tagScreening($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagScreening = new \think\template\taglib\zan\TagScreening;';
    ...
    /**
     * memberlist 调用会员列表
     * {zan:memberlist loop='1' titlelen='20'}
    ...
    public function tagMemberlist($tag, $content)
    {
        ...
        $parseStr .= ' $tagMemberlist = new \think\template\taglib\zan\TagMemberlist;';
    ...
    public function tagMemberinfos($tag, $content)
    {
        ...
        $parseStr .= ' $tagMemberinfos = new \think\template\taglib\zan\TagMemberinfos;';
    ...
    public function tagDiyurl($tag)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagDiyurl = new \think\template\taglib\zan\TagDiyurl;';
     ...
     * 格式：
     * {zan:likearticle mytypeid='0' limit='0,10' titlelen='30' bodylen='160' id='field'}
     * <a href="{$field:arcurl}">{$field:title}</a>
     * {/zan:likearticle}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagLikearticle($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagLikearticle = new \think\template\taglib\zan\TagLikearticle;';
     ...
     * {zan:hotwords num='6' subday='365' maxlength='20'}
    ...
    public function tagHotwords($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagHotwords = new \think\template\taglib\zan\TagHotwords;';
     ...
     * 格式：
     * {zan:weapptaglib name="userList" id="user" empty=""}
     * {user.username}
     * {user.email}
     * {/zan:weapptaglib}
     * @access public
    ...
    public function tagSpecnodelist($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSpecnodelist = new \think\template\taglib\zan\TagSpecnodelist;';
    ...
    public function tagSpecnode($tag, $content)
    {
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagSpecnode = new \think\template\taglib\zan\TagSpecnode;';
     ...
     * pagespecnode 标签解析
     * 在模板中获取专题节点的分页
     * 格式： {zan:pagespecnode listitem='info,index,end,pre,next,pageno' listsize='2'/}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagPagespecnode($tag)
    {
        ...
        $parseStr .= ' $tagPagespecnode = new \think\template\taglib\zan\TagPagespecnode;';
     ...
     * collect
     * 文档收藏
     * 格式： {zan:collect aid='' /}{/zan:collect}
     * class属性 默认关闭off
     * @access public
     * @param array $tag 标签属性
     * @return string
    ...
    public function tagCollect($tag,$content)
    {
        ...
        $parseStr .= ' $tagCollect = new \think\template\taglib\zan\TagCollect;';
    ...
    public function tagNotice($tag,$content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagNotice = new \think\template\taglib\zan\TagNotice;';
    ...
    public function tagAsklist($tag, $content)
    {
        ...
        $parseStr .= ' $tagAsklist = new \think\template\taglib\zan\TagAsklist;';
     ...
     * comment
     * 调用商品评价
     * 格式： {zan:comment aid='' /}{/zan:comment}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagComment($tag,$content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $aid = '.$aid.';';
        $parseStr .= ' $tagComment = new \think\template\taglib\zan\TagComment;';
     ...
     * 格式：
     * {zan:commentlist name="comment_data" id="field"}
     * {user.nickname}
     * {user.level_name}
     * {/zan:commentlist}
     * @access public
     ...
     * {zan:citysite type='son' loop='10' pid='0' empty='' name='' id='' key='' titlelen='' offset='' mod='' currentclass='active'}
     *  <li><a href='{$field:typelink}'>{$field:typename}</a> </li> 
     * {/zan:citysite}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagCitysite($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagCitysite = new \think\template\taglib\zan\TagCitysite;';
    ...
    public function tagArticlepay($tag,$content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagArticlepay = new \think\template\taglib\zan\TagArticlepay;';
    ...
    public function tagDownloadpay($tag,$content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagDownloadpay = new \think\template\taglib\zan\TagDownloadpay;';
    ...
    public function tagNavigation($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            // 查询数据库获取的数据集
            $parseStr .= ' $tagNavigation = new \think\template\taglib\zan\TagNavigation;';
     ...
     * 格式：
     * {zan:formreply name="comment_data" id="field"}
     * {user.nickname}
     * {user.level_name}
     * {/zan:formreply}
     * @access public
    ...
    public function tagFormreply($tag, $content)
    {
        ...
        $parseStr .= ' $tagFormreply = new \think\template\taglib\zan\TagFormreply;';
     ...
     * @return string
     */
    public function tagFaq($tag, $content)
    {
        $group_id  = !empty($tag['group_id']) ? $tag['group_id'] : '0';// 返回的变量group_id
        $group_id  = $this->varOrvalue($group_id);
        $id     = isset($tag['id']) ? $tag['id'] : 'field';
        $orderby = !empty($tag['orderby']) ? $tag['orderby'] : ''; //排序
        if (isset($tag['loop'])) $tag['row'] = $tag['loop'];
        $row = !empty($tag['row']) ? $tag['row'] : 10;
        ...
        // 查询数据库获取的数据集
        $parseStr .= ' $tagFaq = new \think\template\taglib\zan\TagFaq;';
        $parseStr .= ' $_result = $tagFaq->getFaq('.$group_id.', "'.$where.'", "'.$orderby.'");';
        $parseStr .= ' if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $' . $key . ' = 0; $e = 1;';
        // 设置了输出数组长度
        if ('null' != $row) {
            $parseStr .= '$__LIST__ = is_array($_result) ? array_slice($_result,0, '.$row.', true) : $_result->slice(0, '.$row.', true); ';
