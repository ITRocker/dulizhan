...
class TagType extends Base
{
     ...
     */
    public function getType($modelid = '', $typeid = '', $type = 'self', $addfields = '', $empty = '')
    {
        $typeid = !empty($typeid) ? $typeid : $this->tid;

        if (empty($typeid) && empty($modelid)) {
            if (empty($empty)) {
        ...
        } else if (!empty($modelid)) {
            if (1 == $modelid) {
                $ctl_name = 'Article';
                $typename = lang('crumb49', [], self::$home_lang);
            } else if (2 == $modelid) {
                $ctl_name = 'Product';
                $typename = lang('crumb48', [], self::$home_lang);
            } else if (3 == $modelid) {
                $ctl_name = 'Images';
                $typename = lang('crumb57', [], self::$home_lang);
            } else {
                echo '标签type报错：modelid不存在。';
                return false;
            }
            $result = [
                'typename' => $typename,
                'typeurl' => typeurl("home/{$ctl_name}/alls"),
                'current_channel' => $modelid,
            ];
        }
