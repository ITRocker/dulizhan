...
class TagDiyurl extends Base
{
        ...
        if (empty($parseStr)) {
            switch ($type) {
                ...
                case "sindex":     // 搜索主页
                    ...
                    break;
                case "langsite":     // 多语言站点主页
                    $domain = preg_replace('/^(http(s)?:)?(\/\/)?([^\/\:]*)(.*)$/i', '${1}${3}${4}', tpCache('web.web_basehost'));
                    $parseStr = url('home/Langsite/index', [], true, $domain);
                    break;
                default:
                    {
                        if (stristr($link, '/')) {
    ...
    private function DynamicURL($DynamicURL = '', $ParamNew = []) 
    {
        ...
        if (!isset($ParamNew['lang'])) {
            $ParamNew['lang'] = self::$home_lang;
        }
        $ReturnUrl .= '&' . http_build_query($ParamNew);
        // 返回URL
