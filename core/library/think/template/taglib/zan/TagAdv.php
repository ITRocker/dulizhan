...
class TagAdv extends Base
{
    ...
    public function getAdv($pid = '', $where = '', $orderby = '')
    {
        ...
        if (self::$language_split) {
            $langArr = Db::name('ad_position')->where(['id'=>$pid, 'lang'=>self::$home_lang])->cache(true, EYOUCMS_CACHE_TIME, 'ad')->column('lang');
        ...
        if (empty($result) || 'rand' == $orderby) {
            if (empty($where)) { // 新逻辑
                ...
                $result = M("ad_position")->alias('a')
                    ->field("b.*")
                    ->join('__AD__ b', 'a.id = b.pid AND a.lang = b.lang AND b.status = 1', 'LEFT')
                    ->where($where)
                    ->orderRaw($orderby)
                    ->cache(true,EYOUCMS_CACHE_TIME,"ad") // 如果查询条件有时间字段，一定要去掉这行，避免产生一堆缓存文件
                    ->select();
            } else {
                $adpRow = M("ad_position")->where(['id'=>$pid, 'lang'=>self::$home_lang, 'status'=>1])->count();
                if (empty($adpRow)) {
                    return false;
                }
                // 排序
