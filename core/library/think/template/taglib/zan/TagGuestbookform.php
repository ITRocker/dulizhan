    ...
    public function getGuestbookform($typeid = '', $type = 'default', $beforeSubmit = '',$form_type = 0)
    {
        ...
        if (1 == $this->form_type) {
            ...
            if (!empty($typeid)) {
                if (self::$language_split) {
                    $langArr = Db::name('form')->where(['form_id'=>$typeid])->cache(true, EYOUCMS_CACHE_TIME, 'form')->column('lang');
        ...
        if (empty($attributeList)) {
            ...
            foreach ($attributeList as $key => $val) {
                ...
                /*--end*/
                $newAttribute['required_'.$attr_id] = $val['required']; // 是否必填
                $newAttribute['itemname_'.$attr_id] = $val['attr_name']; // 提示文字
                $newAttribute['placeholder_'.$attr_id] = $val['placeholder']; // 表单提示文字
                ...
                if (!empty($val['required'])){
                    ...
                    if ($val['attr_input_type'] == 4) { // 多选框
                        $alert_msg = lang('gbook40', [$val['attr_name']], self::$home_lang);
                        ...
                        $alert_msg = lang('gbook41', [$val['attr_name']], self::$home_lang);
                        $alert_msg = str_replace("'", "\'", $alert_msg);
                        $check_js .= "
                            if(x[i].name == 'attr_".$val['attr_id']."'){
                                var names = document.getElementsByName('attr_".$val['attr_id']."');    
                        ...
                        $alert_msg = lang('gbook34', [$val['attr_name']], self::$home_lang);
                        $alert_msg = str_replace("'", "\'", $alert_msg);
                        $check_js .= "
                ...
                if (!empty($val['validate_type']) && !empty($validate_type_list[$val['validate_type']]['value'])){
                    $alert_msg = lang('gbook35', [$val['attr_name']], self::$home_lang);
                    $alert_msg = str_replace("'", "\'", $alert_msg);
                    $check_js .= " 
            ...
            if (!empty($realValidateData)) {
                ...
                $mobile_alert_msg = str_replace("'", "\'", lang('gbook15', [], self::$home_lang));
                $verify_alert_msg = str_replace("'", "\'", lang('gbook36', [], self::$home_lang));
...
<script type="text/javascript">
    function realValidatePhoneClick(id) {
        ...
        ajax.open("post", "{$this->root_dir}/index.php?m=api&c=Ajax&a=SendMobileCode&lang={$this->lang}", true);
...
EOF;
            ...
            $gbook41 = lang('gbook41', [], self::$home_lang);
	    $gbook41 = str_replace("'", "\'", $gbook41);
            $tokenStr = <<<EOF
<script type="text/javascript">
    ...
    {
        var ajax = new XMLHttpRequest();
        ajax.open("post", "{$this->root_dir}/index.php?m=api&c=Ajax&a=get_token&lang={$this->lang}", true);
        ajax.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    ...
    function getNext1598839807(id,name,level) {
        ...
        if (value) {
            ...
            {
                document.getElementById(id).options.add(new Option('{$gbook41}','')); 
                var ajax = new XMLHttpRequest();
                ajax.open("post", "{$this->root_dir}/index.php?m=api&c=Ajax&a=get_region&lang={$this->lang}", true);
                ajax.setRequestHeader("X-Requested-With","XMLHttpRequest");
                ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
                ajax.send("pid="+value);
                ajax.onreadystatechange = function () {
...
EOF;
            // 查询当前域名是否为绑定的二级域名
            $isOpen = Db::name('language')->where('mark', $home_lang)->getField('is_open');
            ...
            if (empty($isOpen)) {
                $hidden .= '<input type="hidden" name="lang" value="'.$home_lang.'" />';
            }
            $hidden .= $tokenStr;
            $newAttribute['hidden'] = $hidden;

            $action = $this->root_dir."/index.php?m=home&c=Lists&a=gbook_submit";
            if (empty($isOpen)) {
            ...
            $where = [
                'lang' => self::$home_lang,
                'form_id' => intval($typeid)
            ];
            $formArr = Db::name('form')->field('intro, form_name')->where($where)->find();
            $newAttribute['intro'] = !empty($formArr['intro']) ? trim($formArr['intro']) : '';
            $newAttribute['form_name'] = !empty($formArr['form_name']) ? trim($formArr['form_name']) : '';
    ...
    private function getAutoAttrInput($typeid, $newAttribute = [], $attributeList = [], $form_type = 0)
    {
        ...
        foreach($attributeList as $key => $val)
        {
            ...
            switch ($val['attr_input_type']) {
                case '1': // 下拉框
                    $msg = lang('gbook41', [], self::$home_lang);
                ...
                case '6': // 手机号码
                    ...
                    if (!empty($val['real_validate'])) {
                        $verifmsg = lang('gbook42', [], self::$home_lang);
                        $phonemsg = lang('gbook17', [], self::$home_lang);
                        $phonecodemsg = lang('gbook43', [], self::$home_lang);
                        $verifyImgTitle = lang('gbook44', [], self::$home_lang);
                        $verifyImgBtn = lang('gbook45', ['<a class="eyou_form_verify_a" '.$newAttribute['realValidate']['verifyClick'].'>', '</a>'], self::$home_lang);
