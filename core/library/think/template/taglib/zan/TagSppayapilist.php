...
{ 
    ...
    public function getSppayapilist()
    {
        ...
        if (is_array($payData) && (!empty($order_id) || !empty($money_id)) && (!empty($money_code) || !empty($order_code))) {
            if (!empty($money_id)) {
                ...
                if (!empty($payData['type']) && 8 == $payData['type']) {
                    ...
                    if (empty($Result)) $this->error('订单不存在或已变更', $url);
        ...
        $isPaypal = 0;
        if (!empty($PayApiList)) {
            $tagStatic = new \think\template\taglib\zan\TagStatic;
        ...
        if (empty($PayApiList)) return [];

        // 传入JS参数
        $JsonData['IsMobile']        = isMobile() ? 1 : 0;
        $JsonData['is_wap']          = isWeixin() || isMobile() ? 1 : 0;
        $JsonData['PayDealWith']     = dynamic_url('user/Pay/pay_deal_with', ['_ajax' => 1]);
        $JsonData['get_token_url']   = dynamic_url('api/Ajax/get_token', ['name'=>'__token__']);
        $JsonData['SelectPayMethod'] = dynamic_url('user/PayApi/select_pay_method', ['_ajax' => 1], true, true);
        $JsonData['OrderPayPolling'] = dynamic_url('user/PayApi/order_pay_polling', ['_ajax' => 1], true, true);
        $JsonData['UsersUpgradePay'] = dynamic_url('user/PayApi/users_upgrade_pay', ['_ajax' => 1], true, true);
        $JsonData['submitForm']      = '';
        if (!empty($JsonData) && !empty($paypalBusiness)) {
            $url = "https://www.paypal.com/cgi-bin/webscr";
            $return = dynamic_url('plugins/Paypal/paypalNotifyHandlePay', ['transaction_type' => 2, 'is_notify' => 2], true, true);
            $notify_url = dynamic_url('plugins/Paypal/paypalNotifyHandlePay', ['transaction_type' => 2, 'is_notify' => 1], true, true);
            $cancel_return = request()->domain() . ROOT_DIR;
            $currencyCode = tpCache('web.web_currency_unit');
            $currencyCode = !empty($currencyCode) ? trim($currencyCode) : 'USD';
