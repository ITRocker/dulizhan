    ...
    public function getFormreply($typeid = 0, $page = 1,$pagesize = 10, $ordermode = 'desc')
    {
        ...
        if (!empty($result)) {
            ...
            $row       = Db::name('guestbook_attribute')
                ...
                ->alias('a')
                ->join('__GUESTBOOK_ATTR__ b', 'b.attr_id = a.attr_id AND a.lang = b.lang', 'LEFT')
                ->where($where)
            ...
            if (!empty($attr_list)){
                ...
                foreach ($attr_list as $k => $v){
                    if (count($attr_list) == $k + 1){
                        $v['last_one'] = 1;
                        $v['onclick'] = " onclick='get_formreply_list(this);' data-lang='{$this->lang}' data-typeid='{$typeid}' data-page='1' data-totalpage='{$pages['last_page']}' data-pagesize='{$pagesize}' data-ordermode='{$ordermode}' ";
