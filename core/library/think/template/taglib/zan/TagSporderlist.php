        ...
        if (!empty($result['list'])) {
            ...
            foreach ($result['list'] as $key => $value) {
                ...
                if (empty($value['order_status'])) {
                    ...
                    // 跳转链接
                    $result['list'][$key]['PaymentUrl'] = urldecode(dynamic_url('user/Shop/shop_payment_page', ['paystr' => $payStr]));
                    // 手机端微信支付(JSAPI支付)
                ...
                if (!empty($value['payment_method']) && !empty($value['pay_name'])) {
                    ...
                    if (!empty($value['pay_name'])) {
                        ...
                        $result['list'][$key]['pay_name'] = lang('users334', [], $this->home_lang);
                    }
                }
...
EOF;
    ...
    public function getSpstatus()
    {
        ...
        $result['All'] = $result['PendingPayment'] = $result['PendingDelivery'] = $result['PendingReceipt'] = $result['Completed'] = $result['waitComment'] = 0;
