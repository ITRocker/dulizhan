    ...
    public function getTagarclist($param = array(),  $limit = 20, $orderby = '', $addfields = '', $ordermode = '', $tagid = '', $tag = '', $pagesize = 0, $thumb = '', $arcrank = '')
    {
        ...
        foreach (array('keyword','flag','noflag') as $key) {
            if (isset($param[$key]) && $param[$key] !== '') {
                if ($key == 'keyword' && !empty($param[$key])) {
        ...
        $archivesRow = Db::name('archives_'.self::$home_lang)->where(['aid'=>['IN', $aid_arr]])->getAllWithIndex('aid');
        ...
        if (5 == $channeltype) {
            ...
            $resultExt = M($addtableName)->field("aid {$addfields}")->where('aid','in',$aid_arr)->getAllWithIndex('aid');
            /*自定义字段的数据格式处理*/
        ...
        } else if (!empty($addfields) && !empty($aid_arr)) {
            $addtableName = $channeltype_table.'_content_'.self::$home_lang;
            $addfields = str_replace('，', ',', $addfields); // 替换中文逗号
            $addfields = trim($addfields, ',');
            /*过滤不相关的字段*/
