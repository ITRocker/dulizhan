...
class TagSporder extends Base
{
    ...
    public function getSporder($order_id)
    {
        ...
        if (!empty($order_id)) {
            ...
            if (empty($result['OrderData'])) $this->error(lang('users332', [], $this->home_lang));
            ...
            if (!empty($result['OrderData'])) {
                ...
                foreach ($result['DetailsData'] as $key => $value) {
                    ...
                    if (!empty($array_new[$value['product_id']]) && 0 == $value['is_del']) {
                        ...
                        $has_deleted = 1;
                        $msg_deleted = lang('users331', [], $this->home_lang);
                    }
                ...
                if (empty($result['OrderData']['order_status'])) {
                    ...
                    // 跳转链接
                    $result['OrderData']['PaymentUrl'] = urldecode(dynamic_url('user/Shop/shop_payment_page', ['paystr' => $payStr]));
                }
                ...
                if (!empty($result['OrderData']['payment_method'])) {
                    ...
                    $pay_name = lang('users333', [], $this->home_lang);
