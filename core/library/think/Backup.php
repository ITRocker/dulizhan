...
class Backup{
    ...
    private function open($size){
        if($this->fp){
            ...
            if ('install' == $this->file['name']) {
                $filename   = "{$backuppath}zancms.sql";
            } else {
    ...
    public function create(){
        ...
        $sql  = "-- ----------------------------------------\n";
        $sql .= "-- ZanCms MySQL Data Transfer \n";
        $sql .= "-- \n";
    ...
    public function backup($table, $start){
        $des_table = $table;
        if ('install' == $this->file['name']) {
            $zan_prefix = config('database.prefix');
            if ('zan_' != $zan_prefix) {
                $des_table = preg_replace("/^{$zan_prefix}/", "zan_", $des_table);
            }
        }
        //备份表结构
        if(0 == $start){
            ...
            if ('install' == $this->file['name']) {
                if ('zan_' != $zan_prefix) {
                    $create_table = str_replace("`{$zan_prefix}", "`zan_", $create_table);
                }
            }
            $sql .= $create_table . ";\n\n";
                    ...
                    default :
                    {
                        if($flage)
                        {
                            ...
                            if(substr($line,-1)==";")
                            {
                                ...
                                /*--end*/
                                if ('zan_' != config('database.prefix')) {
                                    $sql = str_replace("`{$zan_prefix}", '`'.config('database.prefix'), $sql);
                                }
    ...
    public function sql_split($sql, $tablepre) {
        ...
        if ($tablepre != "zan_")
            $sql = str_replace("`zan_", '`'.$tablepre, $sql);
