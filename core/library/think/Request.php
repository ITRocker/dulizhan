...
class Request
{
     ...
     * @return string
     */
    public function domainPrefix($domainPrefix = '', $ignoreIP = true, $host = '')
    {
        $prefix = '';
        if (!empty($domainPrefix)) {
            $subDomain = $this->subDomain('', $ignoreIP, $host);
            if (empty($subDomain) || !stristr($subDomain, '.')) {
                $prefix = $domainPrefix;
            } else {
                $prefix = preg_replace('/^([^\.]+)\.(.*)$/i', $domainPrefix.'.${2}', $subDomain);
            }
            return $this->scheme().'://'.$prefix.'.'.$this->rootDomain($host);
        }

        $subDomain = $this->subDomain($domainPrefix, $ignoreIP, $host);
        if (stristr($subDomain, '.')) {
            preg_match_all('/^([a-z]+)\.(.*)$/i', $subDomain, $matchs);
            $prefix = empty($matchs[1][0]) ? '' : $matchs[1][0];
        } else {
            $prefix = $subDomain;
        }
    ...
    public function subDomain($domainValue = '', $ignoreIP = true, $host = '')
    {
        ...
        $host = preg_replace('/^(http(s)?:)?(\/\/)?([^\/\:]*)(.*)$/i', '${4}', $host);
