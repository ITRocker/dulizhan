...
class Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        // 后台URL显示语言(编辑切换时使用)
        $this->show_lang = input('param.showlang/s', $this->admin_lang);
        $this->show_lang = preg_replace('/([^a-z])/i', '', $this->show_lang);
        // 系统版本号
        ...
        $this->assign('show_lang', $this->show_lang);
    ...
    private function pc_to_mobile($request = null)
    {
         ...
         *  例如：域名www.zan.com.cn  手机端为m
         *  $request->subDomain() 的值为 "m.zan" != 'm' , 导致下面判断不断得跳转 header('Location: '.$mobileurl)
         *  死循环
         */
