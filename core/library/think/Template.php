...
class Template
{
    ...
    protected $config = [
        ...
        'taglib_pre_load'    => 'zan', // 需要额外加载的标签库(须指定标签库名称)，多个以逗号分隔
    ...
    {
        // 兼容eyou标签语法
        $content = preg_replace('/{(\s*)(\/)?(\s*)eyou(\s*)\:/i', '{${2}zan:', $content);
        $content = preg_replace('/\$eyou([\.|\[]+)/i', '$zan${1}', $content);
        // 判断是否启用布局
    ...
    public function parse(&$content)
    {
        ...
        // 检查zan:include语法  by 小虎哥
        $this->parseZanInclude($content);
        // 检查include语法
        $this->parseInclude($content);
        // 替换包含文件中literal标签内容
        $this->parseLiteral($content);
        // 替换包含文件中zan:literal标签内容
        $this->parseZanLiteral($content);
        // 检查PHP语法
        ...
        // 还原被替换的zan:Literal标签
        $this->parseZanLiteral($content, true);
    ...
    private function parsePhp(&$content)
    {
        ...
        if (isset($view_replace_str['__EVAL__'])) {
            if (stristr($content, '{zan:php}')) { // 针对{zan:php}标签语法处理
                preg_match_all('/{zan\:php}.*{\/zan\:php}/iUs', $content, $matchs);
                $matchs = !empty($matchs[0]) ? $matchs[0] : [];
                if (!empty($matchs)) {
                    foreach($matchs as $key => $val){
                        $valNew = preg_replace('/{(\/)?zan\:php}/i', '', $val);
                        $valNew = preg_replace("/([\W]+)eval(\s*)\(/i", 'intval(', $valNew);
                        $valNew = preg_replace("/^eval(\s*)\(/i", 'intval(', $valNew);
                        $valNew = "{zan:php}{$valNew}{/zan:php}";
    ...
    /**
     * 解析模板中的zan:include标签 by 小虎哥
     * @access private
     * @param  string $content 要解析的模板内容
     * @return void
     */
    private function parseZanInclude(&$content)
    {
        $regex = $this->getRegex('zan:include');
                ...
                foreach ($matches as $match) {
                    ...
                    if ('plugins' == MODULE_NAME && preg_match('/\/template\/(pc|mobile)\//i', $file)) {
                        ...
                        if (preg_match('/^([^\\/]+)(.*)\.'.$this->config['view_suffix'].'$/i', $file) === 1) {
                            // 支持模板引入的写法： {zan:include file="header.htm" /} 或者 {zan:include file="header" /}  或者 {zan:include file="users/header.htm" /} 或者 {zan:include file="users/header" /}
                            ...
                            // 支持绝对路径的模板引入写法： {zan:include file="/template/pc/header.htm" /}
                            $file  = '.'.$file;
     ...
     * @return void
     */
    private function parseZanLiteral(&$content, $restore = false)
    {
        $regex = $this->getRegex($restore ? 'restoreliteral' : 'zan:literal');
        if (preg_match_all($regex, $content, $matches, PREG_SET_ORDER)) {
            if (!$restore) {
                $count = count($this->literal);
                // 替换literal标签
