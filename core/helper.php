...
if (!function_exists('lang')) {
    ...
    {
        // if (in_array($name, ['page1','page2','page3','page4','page5'])) {
        //     if (preg_match('/^page(\d+)$/i', $name)) {
        //         $name = str_replace('page', 'sys', $name);
        //     }
        // }
...
/*
        ...
        if (!empty($lang_switch_on)) {
            if (('lang' == $key || preg_match('/^lang\//i', $key)) && in_array($method, ['param','get']) && stristr(request()->baseFile(), 'index.php')) {
                $current_lang = '';
                // 兼容伪静态多语言切换
                ...
                if (!empty($pathinfo)) {
                    ...
                    $count = \think\Db::name('language')->where(['mark'=>$s_arr[0]])->cache(true, EYOUCMS_CACHE_TIME, 'language')->count();
                    if (!empty($count)) {
                ...
                //     null === $web_basehost && $web_basehost = tpCache('web.web_basehost', [], get_default_lang());
                //     $web_subDomain = request()->domainPrefix('', true, $web_basehost);
                //     $subDomain = request()->domainPrefix();
                //     if (!empty($subDomain) && $web_subDomain != $subDomain) {
                //         $langInfo = \think\Db::name('language')->where('domain',$subDomain)->cache(true, EYOUCMS_CACHE_TIME, 'language')->find();
                //         if (!empty($langInfo['is_open'])) {
                //             $current_lang = $langInfo['mark'];
                //         }
                //     }
                // }
                ...
                if (isset($data['lang'])) {
                    $lang = trim($data['lang'], '/');
                    $lang = trim($lang);
                    empty($lang) && $data['lang'] = $current_lang;
                } else if (is_string($data)) {
                    $lang = trim($data, '/');
                    $lang = trim($lang);
                    empty($lang) && $data = $current_lang;
                }
            }
        }
*/
...
if (!function_exists('custom_url')) {
     ...
     * @return string
     */
    function custom_url($url = '', $vars = [], $suffix = true, $domain = false, $seo_pseudo = null, $seo_pseudo_format = null, $seo_inlet = null)
    {
        $lang = input('lang/s', '');
        if (empty($vars['lang']) && !empty($lang)) $vars['lang'] = trim($lang);
...
if (!function_exists('showlang_url')) {
    /**
     * Url生成
     * @param string        $url 路由地址
     * @param string|array  $vars 变量
     ...
     * @return string
     */
    function showlang_url($url = '', $vars = [], $suffix = true, $domain = false, $seo_pseudo = null, $seo_pseudo_format = null, $seo_inlet = null)
    {
        $lang = input('lang/s', '');
        if (empty($vars['lang']) && !empty($lang)) $vars['lang'] = trim($lang);

        $showlang = input('showlang/s', '');
        if (empty($vars['showlang']) && !empty($showlang)) $vars['showlang'] = trim($showlang);
