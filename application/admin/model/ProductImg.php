...
class ProductImg extends Model
{
     ...
     */
    public function getProductImg($aid = 0, $field = '*')
    {
        // 查询产品图集主表
        $where = [
            'aid' => intval($aid),
        ];
        $result = Db::name('product_img')->field($field)->where($where)->order('sort_order asc')->select();
     ...
     * @author 小虎哥 by 2018-4-3
     */
    public function delProductImg($aid = [])
    {
        if (!is_array($aid)) $aid = array($aid);
        $where = [
            'aid' => ['IN', $aid]
        ];
        return Db::name('product_img')->where($where)->delete(true);
    }
     ...
     * @author 小虎哥 by 2018-4-3
     */
    public function saveProductImg($post = [])
    {
        $proimg = isset($post['proimg']) ? $post['proimg'] : [];
        $imgintro = isset($post['imgintro']) ? $post['imgintro'] : [];

        // 删除产品原来的图片
        if (!empty($post['aid'])) $this->delProductImg($post['aid']);
        ...
        if (!empty($proimg) && count($proimg) > 1) {
            ...
            // 删除缩略图
            delFile(UPLOAD_PATH . "product/thumb/" . $post['aid']);
        }
    }
}