...
class EyouCmsLogic extends Model
{
    ...
    public function welcome_default(&$assign_data = [], $globalConfig = [], $usersConfig = [])
    {
        /*询盘统计、访问用户统计*/
        // 查询访问用户统计
        $visit_log = Db::name('visit_log')->field('auto_id, count, add_time')->select();

        // 查询内容统计
        $where = [
            'form_type' => 1
        ];
        $guestbook = Db::name('guestbook')->field('aid, is_read, add_time')->where($where)->select();
        ...
        foreach ($datesArr as $key => $value) {
            $times = strtotime(date('Y-') . $value);
            $timesArr[] = [$times, $times + 86399];
        }

        // 获取15天内容数据
        $visitLogCount = [];
        $guestbookCount = [];
        foreach ($timesArr as $key_1 => $value_1) {
            $visitLogCount[$key_1] = 0;
            foreach ($visit_log as $key_3 => $value_3) {
                if (intval($value_1[0]) <= intval($value_3['add_time']) && intval($value_1[1]) >= intval($value_3['add_time'])) $visitLogCount[$key_1] = intval($value_3['count']);
            }

            $guestbookCount[$key_1] = 0;
            foreach ($guestbook as $key_2 => $value_2) {
                if (intval($value_1[0]) <= intval($value_2['add_time']) && intval($value_1[1]) >= intval($value_2['add_time'])) $guestbookCount[$key_1]++;
            }
        }
        $visitLogCount = !empty($visitLogCount) ? json_encode($visitLogCount) : '';
        $guestbookCount = !empty($guestbookCount) ? json_encode($guestbookCount) : '';

        // 当月第一天时间戳
        $firstDayTimes = strtotime(date('Y-m'));
        // 当月最后一天时间戳
        $lastDayTimes = strtotime(date('Y-m-t 23:59:59', $firstDayTimes));

        // 累计访问用户、本月访问用户
        $visitTotalCount = $visitMonthCount = 0;
        foreach ($visit_log as $key => $value) {
            // 累计访问用户
            $visitTotalCount += $value['count'];
            // 本月访问用户
            if (intval($firstDayTimes) <= intval($value['add_time']) && intval($lastDayTimes) >= intval($value['add_time'])) $visitMonthCount += $value['count'];
        }

        // 总询盘数、当月询盘数、未读询盘数
        $totalCount = $monthCount = $unreadCount = 0;
        foreach ($guestbook as $key => $value) {
            // 总询盘数
            $totalCount++;
            // 未读询盘数
            if (isset($value['is_read']) && 0 === intval($value['is_read'])) $unreadCount++;
            // 当月询盘数
            if (intval($firstDayTimes) <= intval($value['add_time']) && intval($lastDayTimes) >= intval($value['add_time'])) $monthCount++;
        }
        ...
        $resultData = [
            ...
            'visitTotalCount' => $visitTotalCount,
            'visitMonthCount' => $visitMonthCount,
            // 产品统计
            'archivesCount' => Db::name('archives')->where(['channel' => 2])->count(),
        ];
        $assign_data = array_merge($assign_data, $resultData);
        /*end*/
    ...
    private function contentTotalList()
    {
        ...
        foreach ($quickentryList as $key => $val) {
            ...
            else if ($code == 'Tags@index@') // Tags标签
            {
                ...
                $quickentryList[$key]['total'] = 0; // Db::name('tagindex')->where($map)->count();
            }
    ...
    public function welcome_shop(&$assign_data = [], $globalConfig = [], $usersConfig = [])
    {
        ...
        if (in_array(1,$statistics_type) || in_array(6,$statistics_type)) {
            $archives_data = Db::name('archives')->field('sum(click) as click, count(aid) as total')->where(['is_del'=>0])->find();
        }
        ...
        if (in_array(2,$statistics_type) || in_array(3,$statistics_type)) {
            $shop_order_data = Db::name('shop_order')->field('sum(order_amount) as order_amount, count(order_id) as total')->where(['order_status' => ['IN', [1, 2, 3]]])->find();
        }
        ...
        if (in_array(4,$statistics_type)){
            $total_statistics_data[4] = Db::name('users')->where(['is_del'=>0])->count('users_id');
        }
        if (in_array(5,$statistics_type)){
            $total_statistics_data[5] = Db::name('users_money')->where(['cause_type'=>1,'status' => ['IN', [2, 3]]])->sum('money');
        }
        ...
        //待办事项统计
        $toDoList = [];
        $toDoList['undelivery'] = Db::name('shop_order')->where(['order_status'=>1])->count('order_id');
        $toDoList['service'] = 0; // Db::name('shop_order_service')->where(['status'=>1])->count('service_id');
        $where = [
            'a.is_del' => 0,
            'a.channel' => 2,
            'a.stock_count|b.spec_stock' => ['elt', $usersConfig['goods_stock_warning']],
        ];
        $toDoList['warning'] =  0; // Db::name('archives')->alias('a')->join('product_spec_value b', 'a.aid = b.aid', 'LEFT')->where($where)->group('a.aid')->count('a.aid');
    ...
    public function shouye($globalConfig = [])
    {
        ...
        // --end
        $home_default_lang = get_default_lang();
        ...
        if ($home_default_lang != $this->admin_lang) {
            $langInfo = Db::name('language')->where(['mark'=>$this->admin_lang])->find();
            if (empty($langInfo['is_open'])) {
                ...
                $home_url = $this->request->domainPrefix($langInfo['domain']);
            }
        }
        if (stristr($home_url, '?')) {
            $home_url .= '&clear=1';
