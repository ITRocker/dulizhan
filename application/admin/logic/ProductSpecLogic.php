...
class ProductSpecLogic
{
    ...
    public function SpecAssemblyEdit($GetData = array(), $aid = 0, $quick = false, $isSameLang = '', $inputStyle = '')
    {
        if (empty($isSameLang)) {
            $admin_lang = get_admin_lang();
            $show_lang = input('showlang/s', $admin_lang);
            $isSameLang = trim($admin_lang) === trim($show_lang) ? 1 : 0;
        }
        if (empty($inputStyle) && empty($isSameLang)) $inputStyle = 'disabled style="background-color: #f4f6f8;"';
       ...
       foreach ($spec_arr2 as $k => $v) {
            ...
            $ReturnHtml .="<td><input type='text' class='users_price' name='spec_price[$spec_key][users_price]' value='{$specPrice[$spec_key]['spec_price']}' onpaste='this.value=this.value.replace(/^\D*([0-9]\d*\.?\d{0,2})?.*$/, \"$1\");' onkeyup='this.value=this.value.replace(/^\D*([0-9]\d*\.?\d{0,2})?.*$/, \"$1\"); UpPrice(this);' onchange='setProductSpecPrice();' ".$inputStyle."/></td>";

            $ReturnHtml .="<td><input type='text' class='crossed_price' name='spec_crossed_price[$spec_key][crossed_price]' value='{$specPrice[$spec_key]['spec_crossed_price']}' onpaste='this.value=this.value.replace(/^\D*([0-9]\d*\.?\d{0,2})?.*$/, \"$1\");' onkeyup='this.value=this.value.replace(/^\D*([0-9]\d*\.?\d{0,2})?.*$/, \"$1\"); UpPrice(this);' onchange='setProductSpecPrice();' ".$inputStyle."/></td>";

            $ReturnHtml .="<td><input type='text' class='stock_count' name='spec_stock[$spec_key][stock_count]' value='{$specPrice[$spec_key]['spec_stock']}' onkeyup='this.value=this.value.replace(/[^\d]/g,\"\");UpStock(this);' onpaste='this.value=this.value.replace(/[^\d]/g,\"\")' data-old_stock='{$specPrice[$spec_key]['spec_stock']}' onchange='setProductSpecPrice();' ".$inputStyle."/></td>";

            $ReturnHtml .="<td><input type='text' name='spec_sales[$spec_key][spec_sales_num]' value='{$specPrice[$spec_key]['spec_sales_num']}' onkeyup='this.value=this.value.replace(/[^\d]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d]/g,\"\")' ".$inputStyle."/></td>";
    ...
    public function addProductCustomSpec($post = [])
    {
        ...
        $spec_mark_id = !empty($spec_mark_id) ? intval(++$spec_mark_id) : 1;
        $spec_value_id = Db::name('product_spec_data_handle')->max('spec_value_id');
        $spec_value_id = !empty($spec_value_id) ? intval(++$spec_value_id) : 1;

        // 处理aid和处理规格ID
        $handleAID = $handleSpecID = $handleAutoID = date('His');
        $handleAID = session('handleAID') ? session('handleAID') : $handleAID;
        $handleAID = !empty($post['aid']) ? intval($post['aid']) : $handleAID;
        $handleSpecID = intval($handleSpecID . rand(0, 9));
        $handleAutoID = intval($handleAutoID . rand(10, 99));

        // 添加规格值
        if ('value' === strval($post['action']) && !empty($post['spec_mark_id'])) {
            $where = [
                'aid' => intval($handleAID),
                'spec_mark_id' => $post['spec_mark_id'],
            ];
            $specName = Db::name('product_spec_data_handle')->where($where)->getField('spec_name');
            $insert = [
                'auto_id'        => intval($handleAutoID),
