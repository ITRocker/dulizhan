...
class ArchivesLogic extends Model
{
    ...
    {   
        ...
        foreach ($templateArr as $k2 => $v2) {
            ...
            if (!empty($nid) && 'single' == trim($nid)) {
                preg_match('/^(lists)_'.$nid.'(_(.*))?(_'.$this->admin_lang.')?\.'.$view_suffix.'/i', $v2, $matches1);
            } else {
            ...
            if (!empty($matches1)) {
                if (!empty($nid) && 'single' == trim($nid)) {
                    if ('lists' == $matches1[1]) array_push($templateList, $v2);
                } else {
                    if ('view' == $matches1[1]) array_push($templateList, $v2);
                }
            }
        }
    ...
    public function batch_copy($aids = [], $typeid = 0, $channel = 0, $num = 1)
    {
        ...
        if (!empty($channeltypeRow)) {
            // 主表数据
            $archivesRow = Db::name('archives')->where(['aid'=>['IN', $aids]])->order('aid asc')->select();
            // 内容扩展表数据
            $tableExt = $channeltypeRow['table']."_content";
            $contentRow = Db::name($tableExt)->field('auto_id', true)->where(['aid'=>['IN', $aids]])->getAllWithIndex('aid');

            // 对应语言主表数据
            $archivesRowLang = Db::name('archives_' . $this->admin_lang)->field('auto_id', true)->where(['aid'=>['IN', $aids]])->getAllWithIndex('aid');
            // 对应语言内容扩展表数据
            $tableExtLang = $channeltypeRow['table'] . "_content_" . $this->admin_lang;
            $contentRowLang = Db::name($tableExtLang)->field('auto_id', true)->where(['aid'=>['IN', $aids]])->getAllWithIndex('aid');
            ...
            if ('images' == $channeltypeRow['nid']) { // 图集模型的特性表数据
                ...
                // 产品图集表
                $productParamRow = Db::name('product_param')->field('param_id', true)->where(['aid'=>['IN', $aids]])->select();
                $productParamRow = group_same_key($productParamRow, 'aid');
            }
            ...
            foreach ($archivesRow as $key => $val) {
                ...
                // 自定义地址
                $val['htmlfilename'] = model('Archives')->customRouteHandle(0, preg_replace("/[^\x{4e00}-\x{9fa5}\w\-]+/u", "-", $val['htmlfilename']));

                // 同步数据
                $archivesData = $archivesDataLang = [];
                for ($i = 0; $i < $num; $i++) {
                    // 主表
                    $archivesInfo = $val;
                    $archivesInfo['typeid'] = $typeid;
                    $archivesInfo['stypeid'] = $typeid;
                    $archivesInfo['add_time'] = getTime();
                    $archivesInfo['update_time'] = getTime();
                    unset($archivesInfo['aid']);
                    if (!empty($val['htmlfilename'])) $archivesInfo['htmlfilename'] = $val['htmlfilename'] . $i;
                    $archivesData[] = $archivesInfo;

                    // 对应语言表
                    $archivesInfoLang = !empty($archivesRowLang[$val['aid']]) ? $archivesRowLang[$val['aid']] : $val;
                    $archivesInfoLang['typeid'] = $typeid;
                    $archivesInfoLang['stypeid'] = $typeid;
                    $archivesInfoLang['add_time'] = getTime();
                    $archivesInfoLang['update_time'] = getTime();
                    $archivesInfoLang['old_aid'] = $archivesInfoLang['aid'];
                    unset($archivesInfoLang['aid']);
                    $archivesDataLang[] = $archivesInfoLang;
                }
                ...
                if (!empty($archivesData)) {
                    ...
                    if ($rdata) {
                        ...
                        $productAttrInfo = $productImgInfo = $productNetdiskInfo = $productSpecInfo = $productSpecValueInfo = $shopProductAttrInfo = $productParamInfo = [];
                        $productAttrData = $productImgData = $productNetdiskData = $productSpecData = $productSpecValueData = $shopProductAttrData = $productParamData = [];
                        if ('images' == $channeltypeRow['nid']) { // 图集模型的特性表数据
                            ...
                            // 产品参数表
                            $productParamInfo = !empty($productParamRow[$val['aid']]) ? $productParamRow[$val['aid']] : [];
                        } else if ('media' == $channeltypeRow['nid']) { // 视频模型的特性表数据
                        ...
                        foreach ($rdata as $k1 => $v1) {
                            ...
                            $archivesDataLang[$k1]['aid'] = intval($aid_new);
                            ...
                            if ('images' == $channeltypeRow['nid']) {
                                ...
                                // 产品参数表
                                foreach ($productParamInfo as $param_k => $param_v) {
                                    $param_v['aid'] = $aid_new;
                                    $productParamData[] = $param_v;
                                }
                        ...
                        // 对应语言数据添加
                        $contentDataLang = [];
                        foreach ($archivesDataLang as $key_ => $value_) {
                            $contentInfoLang = !empty($contentRowLang[$value_['old_aid']]) ? $contentRowLang[$value_['old_aid']] : $contentInfo;
                            if (!empty($contentInfoLang)) {
                                $contentInfoLang['aid'] = intval($value_['aid']);
                                $contentDataLang[] = $contentInfoLang;
                            }
                            unset($archivesDataLang[$key_]['old_aid']);
                        }
                        if (!empty($contentDataLang)) Db::name($tableExtLang)->insertAll($contentDataLang);
                        if (!empty($archivesDataLang)) Db::name('archives_' . $this->admin_lang)->insertAll($archivesDataLang);
                        // -------end
                        ...
                        if ('images' == $channeltypeRow['nid']) {
                            ...
                            // 产品参数表
                            if (!empty($productParamData)) {
                                Db::name('product_param')->insertAll($productParamData);
                                if (!empty($markList)) {
                                    foreach ($markList as $langVal) {
                                        if (!empty($langVal)) Db::name('product_param_' . $langVal)->insertAll($productParamData);
                                    }
                                }
                            }
                        } else if ('media' == $channeltypeRow['nid']) {
