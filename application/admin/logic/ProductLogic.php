...
EOF;
    ...
    public function getShopAttrInput($aid, $typeid, $list_id)
    {
        ...
        foreach($attributeList as $key => $val) {
            ...
            foreach($curAttrVal as $k =>$v) {
                $str .= "<dl class='d-flex mb5 attr_{$attr_id}'>";
                $addDelAttr = ''; // 加减符号
                ...
                $str .= "<dt class='w200 pl5'><input type='text' size='10' value='$addDelAttr {$val['attr_name']}' name='attr_{$attr_id}' readonly='readonly' class='w190'/></dt>";
                        
                // 单行文本框
                if($val['attr_input_type'] == 0) {
                    $str .= "<dd class='w400 pl5'><input type='text' size='40' value='".($aid ? $v['attr_value'] : $val['attr_values'])."' name='shop_attr_{$attr_id}[]' class='w350' /><a class='text_a grey' href='javascript:void(0);' >&nbsp;&nbsp;删除</a></dd>";
                }
