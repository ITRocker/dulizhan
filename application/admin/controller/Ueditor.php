...
class Ueditor extends Base
{
    ...
    private function upFile($fieldName) {
        ...
        if (!empty($info)) {
            ...
            $resultArr = [];
            if (!empty($return_url)) {
                $resultArr = autoConvertWebpImages($return_url, UPLOAD_PATH.$this->savePath, $info->getSize());
            ...
            $data = array(
                ...
                'type' => !empty($resultArr['resultInt']) && !empty($resultArr['mimetype']) ? '.' . $resultArr['mimetype'] : '.' . $info->getExtension(),
                'size' => !empty($resultArr['resultInt']) && !empty($resultArr['filesize']) ? $resultArr['filesize'] : $info->getSize(),
            );
    ...
    */
    ...
    private function saveRemote($config,$fieldName){
        ...
        if(!(file_put_contents($fullName, $img) && file_exists($fullName))){ //移动失败
            ...
            $resultArr = [];
            if (!empty($return_url)) {
                $resultArr = autoConvertWebpImages($return_url, UPLOAD_PATH.'ueditor/'.date('Ymd/'), $file['filesize']);
                $return_url = !empty($resultArr['resultInt']) && !empty($resultArr['return_url']) ? $resultArr['return_url'] : $return_url;
            }
    ...
    */
    ...
    public function imageUp()
    {
        ...
        if (true !== $result || empty($file)) {
            ...
            $height = isset($img_info[1]) ? $img_info[1] : 0;
            $mime = isset($img_info['mime']) ? $img_info['mime'] : $info->getMime();
            $filesize = !empty($resultArr['resultInt']) && !empty($resultArr['filesize']) ? $resultArr['filesize'] : $info->getSize();
