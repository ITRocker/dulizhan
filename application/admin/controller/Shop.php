...
class Shop extends Base {
    ...
    public function conf()
    {
        if (IS_POST) {
            ...
            if (!empty($post['shop'])) {
                ...
                foreach ($langRow as $key => $val) {
                    getUsersConfigData('shop', $post['shop'], $val['mark']);
                }
        ...
        $apiConfig = Db::name('pay_api_config')->where($where)->find();
        $apiConfigInfo = !empty($apiConfig['pay_info']) ? unserialize($apiConfig['pay_info']) : [];
        $this->assign('apiConfig', $apiConfig);
        $this->assign('apiConfigInfo', $apiConfigInfo);
    ...
    public function index()
    {
        ...
        foreach ($DetailsData as $key => $value) {
            ...
            $value['arcurl'] = get_arcurl($ArchivesData[$value['product_id']], true, '', $value['lang']);
    ...
    public function order_send_operating()
    {
        if (IS_POST) {
            ...
            if (!empty($IsOrder)) {
                ...
                if (!empty($post['email'])) $Users = ['email' => trim($post['email']), 'nickname' => trim($post['email'])];
    ...
    public function shipping_template()
    {
        ...
        if (!empty($region_name)) $where['a.name'] = array('LIKE', "%{$region_name}%");
        // 省份
        ...
        if (!isset($this->usersConfig['shop_open_shipping_type'])) $this->usersConfig['shop_open_shipping_type'] = 1;
        if (!isset($this->usersConfig['shop_open_shipping_money'])) $this->usersConfig['shop_open_shipping_money'] = 0;
    ...
    private function myShopWeapp(&$assign_data = [])
    {
        ...
        try {
            ...
            $url       = 'http://plugins.zancms.com/user/ajax_memberplugin.php?action=myshop_weapp';
    ...
    public function pluginShopWeapp()
    {
        ...
        $url       = 'http://plugins.zancms.com/user/ajax_memberplugin.php?action=plugin';
        $post_data = [
        ...
        if (!empty($weappArr)) {
            ...
            $url = $upgradeLogic->getServiceUrl(true, 'zan').'/index.php?m=api&c=Weapp&a=checkBatchVersion';
