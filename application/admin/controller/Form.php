...
class Form extends Base
{
    ...
    public function _initialize()
    {
        ...
        // 模型层
        $this->formModel = model('Form');
        // 表单ID
        $this->form_id = input('form_id/d', 1);
        $this->assign('form_id', $this->form_id);
        // 获取左侧列表
        /*$this->formList = $this->formModel->getFormList(get_admin_lang());
        $this->assign('formList', $this->formList);*/
    }
    ...
    {
        $formAttr = $this->formModel->getFormAttrList($this->form_id);
        $this->assign('formAttr', $formAttr);

        $where = [
            'is_read' => 0,
        ];
        $unreadList = Db::name('guestbook')->field('typeid, count(typeid) as count')->where($where)->group('typeid')->getAllWithIndex('typeid');
        $this->assign('unreadList', $unreadList);
        ...
        if (empty($typeid) && !empty($param['form_id'])) $typeid = $param['typeid'] = intval($param['form_id']);
        ...
        // 应用搜索条件
        foreach (['keywords', 'typeid', 'count_type'] as $key) {
            if (isset($param[$key]) && $param[$key] !== '') {
                if ($key == 'keywords') {
                    $aids = Db::name('guestbook_attr')->where(['attr_value' => ['LIKE', "%{$param[$key]}%"]])->group('aid')->column('aid');
...
// dump($assign_data);exit;
    ...
    public function field()
    {
        ...
        // 查询表单填写数量
        $assign_data['form_list_count'] = $this->formModel->GetFormListCount($form_ids);
    ...
    {
        if (IS_AJAX_POST) {
            $this->formModel->saveFormData();
        }

        $info = $this->formModel->getFormFind($this->form_id);
        if (empty($info)) $this->error('表单不存在，请联系管理员！');
        $assign_data['info'] = $info;
        $assign_data['formAttr'] = $this->formModel->getFormAttrList($this->form_id);
        // dump($assign_data);exit;
        $this->assign($assign_data);
        return $this->fetch();
    }
    ...
    {
        if (IS_AJAX_POST) {
            $this->formModel->saveFormAttrData();
        }
        ...
        /*--end*/
        $info = $this->formModel->getFormAttrFind($id);
        if (empty($info)) $this->error('数据不存在，请联系管理员！');
        $assign_data['field'] = $info;

        // 所在表单
        $formdata = $this->formModel->getFormFind($info['typeid']);
        $assign_data['formdata'] = $formdata;
    ...
    public function label_call(){
        ...
        if (!empty($form_id)) {
            ...
            $content =<<<EOF
{zan:form type="auto" formid="{$form['form_id']}" id="field"}
<form method="POST" {\$field.formhidden} action="{\$field.action}" onsubmit="{\$field.submit}">
    {zan:volist name="\$field.attrlist" id="attr"}
