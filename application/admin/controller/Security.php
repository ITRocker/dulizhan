...
class Security extends Base
{
    ...
    public function ddos_kill()
    {
        ...
        $syn_admin_logic_1726216198 = zanSetting('syn.syn_admin_logic_1726216198');
        if (empty($syn_admin_logic_1726216198)) {
            try {
                @Db::execute("DROP TABLE IF EXISTS `{$Prefix}ddos_log`");
                zanSetting('syn', ['syn_admin_logic_1726216198' => 1]);
            } catch (\Exception $e) {}
        }
...
*/
    ...
    public function ddos_scan_file()
    {
        ...
        if (IS_POST) {
            ...
            if ('files' == $scan_range) {
                // $data = $this->ddosLogic->ddosHandelScanFile($doubtotal, $achievepage, $achievefile, $allscantotal, true, 100);
    ...
    public function ddos_replacefile($md5keys = [])
    {
        ...
        if (!empty($md5keys)) {
            $result = Db::name('ddos_log')->field('html', true)->where(['md5key'=>['IN', $md5keys], 'file_grade'=>['gt', 0], 'admin_id'=>$this->admin_id])->select();
            foreach ($result as $key => $val) {
                if (preg_match('/^(template)\/(.*)$/i', $val['file_name'])) {
                    unset($result[$key]);
                }
            }
            if (empty($result)) {
