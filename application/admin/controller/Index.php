...
//        $this->assign('menu',getMenuList());
        ...
        //左侧导航默认固定菜单
        $leftmenu_default_ids = config('global.leftmenu_default_ids');
        $this->assign('leftmenu_default_ids', $leftmenu_default_ids);
    ...
    public function welcome()
    {
        ...
        if (2 <= $this->php_servicemeal) {
            $web_theme_welcome_tplname = 'welcome_shop.htm';
        }
    ...
    public function changeTableVal()
    {
        if (IS_AJAX_POST) {
                ...
                case 'language':
                    {
                        ...
                        if (is_array($return)) {
                            $data = [];
                            isset($return['refresh']) && $data['refresh'] = $return['refresh'];
                            $time = !empty($return['time']) ? $return['time'] : 3;
                            $this->error($return['msg'], null, $data, $time);
                        }
                    }
                    break;
                // 积分商品列表
            ...
            switch ($table) {
                ...
                case 'language':
                {
                    if ('is_home_default' == $field) {
                        if($value == 1) {
                            $savedata['url'] = '';
                            $savedata['is_open'] = 0;
                        }
                        $savedata['is_admin_default'] = $value;
                    }
                    break;
                }
            }
            ...
            // if (in_array($field, ['nav_name', 'typename'])) $where['lang'] = $this->admin_lang;

            // 根据条件保存修改的数据
            $r = Db::name($table)->where($where)->cache(true,null,$table)->save($savedata);
            if ($r !== false) {
                if ('users' == $table && 'is_activation' == $field){
                    $data['refresh'] = 1;
                    $data['time'] = 500;
                }
                ...
                switch ($table) {
                    ...
                    {
                        if ('is_home_default' == $field) {
                            $mark = Db::name($table)->where($where)->value('mark');
                            tpCache('web', ['web_basehost'=>get_domain()], $mark);
                            if (1 == $value) { // 设置默认语言，只允许有一个是默认，其他取消
                                Db::name('language')->where($id_name,'NEQ',$id_value)->update([
                                        'is_home_default' => 0,
                                        'is_admin_default' => 0,
                                        'update_time' => getTime(),
                                    ]);
                                $langRow = Db::name('language')->order('id asc')
                                    ...
                                    ->select();
                                $system_home_default_lang = '';
                                foreach ($langRow as $key => $val) {
                                    if (1 == $val['is_home_default']) {
                                        $system_home_default_lang = $val['mark'];
                                        break;
                                    }
                                }
                                foreach ($langRow as $key => $val) {
                                    tpCache('system', ['system_home_default_lang'=>$system_home_default_lang], $val['mark']);
                                }
                            }
                            $data['openAlert'] = 1;
                            $data['refresh'] = 1;
                            $data['time'] = 500;
                            $data['url'] = url('Language/index', ['lang'=>$system_home_default_lang]);
                        }
                        break;
                    }
    ...
    public function switch_map()
    {
            ...
            switch ($inc_type) {
                ...
                case 'web':
                {
                    ...
                    if (in_array($name, ['web_users_switch'])) {
                        ...
                        if (empty($value)) {
                            foreach ($langRow as $key => $val) {
                                getUsersConfigData('users', ['users_open_release' => 0], $val['mark']); // 会员投稿
                                getUsersConfigData('level', ['level_member_upgrade' => 0], $val['mark']); // 会员升级
                                getUsersConfigData('shop', ['shop_open' => 0], $val['mark']); // 商城中心
                                getUsersConfigData('pay', ['pay_open' => 0], $val['mark']); // 支付功能
                            }
                        }
                    }
                    break;
        ...
        // 已加入左侧菜单的内置功能
        $leftMenuArr = Db::name('admin_menu')->field('menu_id')->getAllWithIndex('menu_id');
        $this->assign('leftMenuArr',$leftMenuArr);
        //左侧导航默认固定菜单
        $leftmenu_default_ids = config('global.leftmenu_default_ids');
        $this->assign('leftmenu_default_ids', $leftmenu_default_ids);
        ...
        $row = Db::name("admin_menu")->field("menu_id,controller_name,action_name,title,icon,is_menu,is_switch")->where($map)->order("sort_order asc,update_time asc,id asc")->select();
        foreach ($row as $key=>$val){
            if (1 == $val['is_menu'] || in_array($val['menu_id'], $leftmenu_default_ids)) {
                $admin_menu_list[] = $val;
    ...
    public function theme_index()
    {
        ...
        if (empty($this->globalConfig['web_theme_styleid'])) $this->globalConfig['web_theme_styleid'] = 1;
        ...
        if (IS_POST) {
            ...
            foreach ($post as $key => $val) {
                ...
                if ('theme_id' == $key) {
                    ...
                    $webData['web_theme_css_tplname'] = 'css_'.intval($val).'.css';
            ...
            if (6 <= $webData['web_theme_styleid'] && 2 > $this->php_servicemeal) {
                $this->error($this->globalConfig['sys_tmpserinfo']['authormsg2']);
            } else if (1 < $webData['web_theme_styleid'] && 1 > $this->php_servicemeal) {
                $this->error($this->globalConfig['sys_tmpserinfo']['authormsg1']);
            }
            if (!empty($webData)) {
                /*多语言*/
                $langRow = \think\Db::name('language')->order('id asc')->select();
    ...
    public function ajax_theme_tplfile_add()
    {
        ...
        if (IS_POST) {
            ...
            if ('htm' == $ext) {
                ...
                if (preg_match('#<([^?]*)\?php#i', $content) || preg_match('#<\?(\s*)=#i', $content) || (preg_match('#<\?#i', $content) && preg_match('#\?>#i', $content)) || preg_match('#\{zan\:php([^\}]*)\}#i', $content) || preg_match('#\{php([^\}]*)\}#i', $content) || preg_match('#(\s+)language(\s*)=(\s*)("|\')?php("|\')?#i', $content)) {
