...
if (!function_exists('tpversion')) 
{
    function tpversion($timeout = 5)
    {
        ...
        $global_config = tpCache('global', [], get_admin_lang());
        $servefunclist = empty($global_config['php_servefunclist']) ? '' : base64_decode($global_config['php_servefunclist']);
        if (!preg_match('/\|install(_true)?\|/i', $servefunclist)) {
            return false;
        }

        $server_ip = serverIP();
        if (!preg_match('/\|install\|/i', $servefunclist) && is_local_ip($server_ip)) {
            return false;
        }
        ...
        $values = array(            
            ...
            'web_title' => $global_config['web_title'],
        );
        // api_Service_user_push
        $upgradeLogic = new \app\admin\logic\UpgradeLogic;
        $upgradeLogic->GetKeyData($values);
        $url = $upgradeLogic->getServiceUrl(true).'/index.php?m=api&c=Service&a=user_push';
        $response = @httpRequest($url, 'POST', $values, [], $timeout);
        if (false === $response) {
            $get_url = $url.'&'.http_build_query($values);
...
if (!function_exists('push_zzbaidu')) 
{
    ...
    {
        // 获取token的值：http://ziyuan.baidu.com/linksubmit/index?site=http://www.zancms.com/
        $aid = intval($aid);
...
if (!function_exists('check_single_uiset')) {
    ...
    function check_single_uiset($templist = '')
    {
        ...
        if (file_exists($file)) {
            ...
            if (!empty($content) && preg_match('/zan\:ui(\s+)open\=(\'|\")(on|off)(\'|\")/i', $content)) {
                $uisetRow['pc'] = true;
            }
        }
...
if (!function_exists('get_not_role_menu_id')){
    ...
    function get_not_role_menu_id(){
        ...
        //有权限的  (1 == tpCache('global.web_users_switch'))
        //会员中心
        if (!empty($module_rele_menu['web_users_switch']) && (1 != tpCache('global.web_users_switch'))){
            $not_role_menu_id = array_merge($not_role_menu_id,$module_rele_menu['web_users_switch']);
        }
        //商城中心 => 会员中心关闭则一起关闭
//        if (1 == tpCache('global.web_users_switch') && 1 == $UsersConfig['shop_open'] && 1.5 <= tpCache($shopServicemeal))
        if (!empty($module_rele_menu['shop_open']) && (1 != tpCache('global.web_users_switch') || 1 != $UsersConfig['shop_open'] || 1.5 > tpCache($shopServicemeal))){
        ...
        //插件应用
        if (!empty($module_rele_menu['web_weapp_switch']) && (1 != tpCache('global.web_weapp_switch'))){
