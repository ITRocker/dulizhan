...
EOF;
    ...
    public function delGoodsSurplusSpec()
    {
        // 删除条件
        $where = [
            // 当前时间 - 一天的时间
            'add_time' => ['<', $this->times - 86400]
        ];
        // 删除多余规格值数据
    ...
    public function getVisitorsID()
    {
        ...
        // $visitorsID = '9' . date('His') . rand(10, 99);
        $visitorsID = '9' . date('md') . rand(1000, 9999);
        $shopVisitorsPay = getUsersConfigData('shop.shop_visitors_pay');
        if (empty($shopVisitorsPay) || !empty($users_id)) {
            $visitorsID = 0;
            if (cookie('visitorsID')) {
                cookie('visitorsID', null);
            }
        } else {
            if (cookie('visitorsID')) {
                $visitorsID = intval(cookie('visitorsID'));
            } else {
                cookie('visitorsID', intval($visitorsID));
            }
        }
        return $visitorsID;
    }
