...
class EmailLogic 
{
    ...
    public function __construct($smtp_config = []) 
    {
        ...
        $this->config = !empty($smtp_config) ? $smtp_config : tpCache('smtp', [], $this->lang);
    }
    ...
    private function send_swiftmailer($to='', $subject='', $data='', $scene=1){
        ...
        try {
            ...
            if (!$result) {
                return array('code'=>0 , 'msg'=>lang('users134', [], $this->lang));
            } else {
                return array('code'=>1 , 'msg'=>lang('users135', [], $this->lang));
            }
        } catch (\Exception $e) {
            return array('code'=>0 , 'msg'=>lang('users134', [], $this->lang).': '.$e->errorMessage());
        }
    }
    ...
    private function send_phpmailer($to='', $subject='', $data='', $scene=1){
        ...
        try {
            ...
            if (!$result) {
                ...
                return array('code'=>0 , 'msg'=>lang('users134', [], $this->lang).'：'.$msg);
            } else {
                return array('code'=>1 , 'msg'=>lang('users135', [], $this->lang));
            }
        } catch (\Exception $e) {
