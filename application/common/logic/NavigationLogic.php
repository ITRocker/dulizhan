...
class NavigationLogic extends Model
{
    ...
    public function GetAllArctype($type_id = 0)
    {
        ...
        // 下拉框拼装
        $HtmlCode = '<select name="type_id" id="type_id" onchange="selectArctype(this);">';
    ...
    public function getAllArctypeList($type_id = 0)
    {
        ...
        // 获取全部可用栏目
        $options = $this->arctypeLogic->arctype_list(0, 0, false, $arctype_max_level, ['is_del' => 0]);
        ...
        // 栏目选择内容
        $arctypeHtml = '<select name="type_id" id="type_id" onchange="selectArctype(this);">';
        $arctypeHtml .= '<option id="arctype_default" value="0">请选择菜单</option>';
        $isSelected_1 = -1 === intval($type_id) ? 'selected' : '';
        $arctypeHtml .= '<option value="-1" data-typeurl="/" data-typename="首页" data-custom="0" '.$isSelected_1.'>首页</option>';
        foreach ($options as $var) {
            ...
            if (2 == $seo_pseudo) {
                ...
                // 动态或伪静态
                $controller_name = $channel_list[$var['current_channel']]['ctl_name'];
                $typeurl = auto_hide_index(typeurl("home/{$controller_name}/lists", $var, true, false, $seo_pseudo, null));
            }
            // 拼装选项
            $arctypeHtml .= '<option value="' . $var['id'] . '" data-typeurl="' . $typeurl . '" data-typename="' . $var['typename'] . '" ' . $isSelected . ' data-custom="0">';
            if ($var['level'] > 0) {
                $arctypeHtml .= str_repeat('&nbsp;', $var['level'] * 4);
            }
            $arctypeHtml .= htmlspecialchars_decode(addslashes($var['typename'])) . '</option>';
        }
        $isSelected_2 = -2 === intval($type_id) ? 'selected' : '';
        $arctypeHtml .= '<option value="-2" data-typeurl="" data-typename="" data-custom="1" '.$isSelected_2.'>自定义</option>';
        $arctypeHtml .= '</select>';
    ...
    // 获取全部导航菜单列表
    public function getAllNavList($position_id = 0, $nav_id = 0, $level = 0)
    {
        // 查询所有可投稿的菜单的顶级菜单
        $where = [
            'c.is_del' => 0,
        ...
        $navList = $this->nav_list(0, 0, false, $level, $where, false);
