...
class PayApiLogic extends Model
{
    ...
    public function GetFindOrderData($post = [], $is_up_order = false, $config = [])
    {
        ...
        // 获取商品订单
            ...
            // 同步处理
            if (empty($this->fromAsync)) {
                if (empty($OrderData)) $this->error(lang('users349', [], $this->home_lang));
                // 判断订单状态，1已付款(待发货)，2已发货(待收货)，3已完成(确认收货)，-1订单取消(已关闭)，4订单过期
                $url = urldecode(custom_url('user/Shop/shop_pay_success', ['order_id' => intval($OrderData['order_id'])]));
                ...
                if (in_array($OrderData['order_status'], [1, 2, 3])) {
                    ...
                    // }
                    $this->success(lang('users346', [], $this->home_lang), $url, true);
                } else if ($OrderData['order_status'] == 4) {
                    $this->success(lang('users347', [], $this->home_lang), $url, true);
                } else if ($OrderData['order_status'] == -1) {
                    $this->success(lang('users348', [], $this->home_lang), $url, true);
                }
                // 更新订单支付方式
                /*if (!empty($is_up_order)) {
                    $update = [
    ...
    public function OrderProcessing($Post = [], $Order = [], $PayDetails = [], $config = [], $queryOrder = true)
    {
        ...
        // 商品订单处理
            ...
            if (empty($Order['order_status'])) {
                ...
                $users_ = !empty($this->visitorsID) && empty($this->users_id) ? ['email' => trim($Order['email']), 'nickname' => trim($Order['email'])] : [];
                $users_id_ = !empty($this->visitorsID) && empty($this->users_id) ? intval($this->visitorsID) : intval($this->users_id);
                $returnData = pay_success_logic($users_id_, $Order['order_code'], $PayDetails, $Order['pay_name'], $async, $users_, $config);
                ...
                if (empty($this->fromAsync)) {
                    $returnData = [];
                    $url = urldecode(custom_url('user/Shop/shop_pay_success', ['order_id' => intval($Order['order_id'])]));
                    if (!empty($this->visitorsID) && empty($Order['users_id'])) {
                        // $langInfo = cookie('lang_info') ? json_decode(cookie('lang_info'), true) : [];
                        // $url = !empty($langInfo['lang_pageurl']) ? trim($langInfo['lang_pageurl']) : ROOT_DIR . '/';
                        $users = ['email' => trim($Order['email']), 'nickname' => trim($Order['email'])];
                    } else {
                    ...
                    $this->success('已支付', $url, $returnData);
                }
            }
        }
        // 会员升级处理
