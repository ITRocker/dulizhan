...
class Base extends Common {
    ...
    public function _initialize() {
        ...
        if (!empty($users_id)) {
            ...
            // dump($this->visitorsID);exit;
            if (!empty($this->visitorsID)) {
                $filter_login_action = array_merge($filter_login_action, [
                    'Shop@cart_del',
                    'Shop@cart_checked',
                    'Shop@shop_buy_now',
                    'Shop@shop_add_cart',
                    'Shop@to_split_goods',
                    'Shop@shop_cart_list',
                    'Shop@select_cart_del',
                    'Shop@shop_order_addr',
                    'Shop@shop_under_order',
                    'Shop@shop_pay_success',
                    'Shop@shop_add_address',
                    'Shop@shop_edit_address',
                    'Shop@shop_payment_page',
                    'PayApi@select_pay_method',
                    'PayApi@order_pay_polling',
                    'Shop@cart_unified_algorithm',
                ]);
            }
        ...
        if ($assignValue2 <= 1) {
            $logut_redirect_url = ROOT_DIR.'/';
            $logut_redirect_url_test  = "该"."功"."能"."为"."商"."城"."版"."本"."可"."用";
        }
