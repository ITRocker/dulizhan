...
class PayApi extends Base {
    ...
    public function select_pay_method()
    {
        ...
        // 订单查询 
        $orderData = $this->PayApiLogic->GetFindOrderData($post, true);

        // 支付API配置信息查询 
        $apiConfig = $this->PayApiLogic->GetPayApiConfig($post);
        if (!empty($apiConfig) && 1 == $apiConfig['pay_id'] && 'wechat' == $apiConfig['pay_mark']) {
            // 系统内置的微信支付
            $this->PayApiLogic->UseWeChatPay($post, $orderData, $apiConfig['pay_info']);
        }
        else if (!empty($apiConfig) && 2 == $apiConfig['pay_id'] && 'alipay' == $apiConfig['pay_mark']) {
            // 系统内置的支付宝支付
            $this->PayApiLogic->UseAliPayPay($post, $orderData, $apiConfig['pay_info']);
        }
        else if (!empty($apiConfig) && !empty($apiConfig['pay_mark']) && 0 == $apiConfig['system_built']) {
            // 第三方插件
            $controllerName  = "\weapp\\" . $apiConfig['pay_mark']."\controller\\" . $apiConfig['pay_mark'];
            $unifyController = new $controllerName;

            // 虎皮椒支付成功后返回页面，主要用于手机浏览器端、微信端使用虎皮椒支付后页面跳转
            $unifiedUrl = custom_url('user/Shop/shop_pay_success', ['order_id' => intval($orderData['order_id'])]);
            ...
            //     $unifiedUrl = cookie($this->users_id . '_' . $orderData['product_id'] . '_EyouMediaViewUrl');
            // } else if (9 == $post['transaction_type']) {
            //     $unifiedUrl = cookie($this->users_id . '_' . $orderData['product_id'] . '_EyouArticleViewUrl');
            // } else if (10 == $post['transaction_type']) {
            //     $unifiedUrl = cookie($this->users_id . '_' . $orderData['product_id'] . '_EyouDownloadViewUrl');
            // }
            // if (!empty($this->visitorsID) && empty($this->users_id)) {
            //     $langInfo = cookie('lang_info') ? json_decode(cookie('lang_info'), true) : [];
            //     $unifiedUrl = !empty($langInfo['lang_pageurl']) ? trim($langInfo['lang_pageurl']) : $unifiedUrl;
            // }
            $resultData = $unifyController->UnifyGetPayAction($apiConfig['pay_info'], $orderData, $unifiedUrl);
            if (!empty($resultData)) {
    ...
    public function order_pay_polling()
    {
        if (IS_AJAX_POST) {
            ...
            // 订单查询 
            $orderData = $this->PayApiLogic->GetFindOrderData($post);
            ...
            if (!empty($apiConfig) && 1 == $apiConfig['pay_id'] && 'wechat' == $apiConfig['pay_mark']) {
                // 系统内置微信支付---微信支付订单处理
                $this->PayApiLogic->WeChatPayProcessing($post, $orderData, $apiConfig['pay_info'], $apiConfig);
            }
            else if (!empty($apiConfig) && 2 == $apiConfig['pay_id'] && 'alipay' == $apiConfig['pay_mark']) {
                // 系统内置支付宝支付---支付宝支付订单处理
                $this->PayApiLogic->AliPayPayProcessing($post, $orderData, $apiConfig['pay_info'], $apiConfig);
            }
            else if (!empty($apiConfig) && !empty($apiConfig['pay_mark']) && 0 == $apiConfig['system_built']) {
                // 第三方支付
                $controllerName  = "\weapp\\" . $apiConfig['pay_mark'] . "\controller\\" . $apiConfig['pay_mark'];
                $unifyController = new $controllerName;
                $resultData = $unifyController->OtherPayProcessing($apiConfig['pay_info'], $post['unified_number'], $post['transaction_type']);
                if (is_array($resultData)) {
                    ...
                    $this->PayApiLogic->OrderProcessing($post, $orderData, $resultData, $apiConfig);
                } else {
