...
class Shop extends Base
{
    ...
    {
        // 隐藏购物车右边栏
        $this->assign('hideCartRight', true);
        // 数据由标签调取生成
    ...
    public function shop_under_order($error = true)
    {
        ...
        if (empty($error)) {
            if ($this->usersTplVersion == 'v3') {
                ...
                $this->error(lang('users58', [], $this->home_lang));
            }
        }
    ...
    public function shop_order_addr()
    {
        if (IS_AJAX_POST) {
            // 国家数据表
            $field['countryList'] = Db::name('country')->field('id, name')->getAllWithIndex('id');
            ...
            $field['shopAddressFind'] = Db::name('shop_address')->where($where)->find();

            // 加载页面返回
            $this->assign('field', $field);
            $html = $this->fetch('users/shop_order_addr');
            $this->success('检测完成', null, $html);
        }
    }
        ...
        if (IS_AJAX_POST) {
            ...
            if (!empty($order_id)) {
                ...
                if (!empty($return)) {
                    ...
                    $this->success(lang('users335', [], $this->home_lang));
                }else{
    ...
    public function shop_buy_now()
    {
        if (IS_AJAX_POST) {
            ...
            if (!empty($count)) {
                ...
                // 跳转链接
                $this->success(lang('sys314', [], $this->home_lang), urldecode(dynamic_url('user/Shop/shop_under_order', ['querystr' => $querystr])));
            } else {
                $this->error(lang('users336', [], $this->home_lang));
            }
        } else {
            $this->error('非法访问！');
        }
    ...
    public function shop_add_cart()
    {
        if (IS_AJAX_POST) {
            ...
            if (!empty($count)) {
                ...
                if (!empty($this->visitorsID) && empty($this->users_id)) $where['users_id'] = intval($this->visitorsID);
                // 若开启多规格则执行
                if (!empty($this->usersConfig['shop_open_spec']) && !empty($param['spec_value_id'])) $where['spec_value_id'] = $param['spec_value_id'];
                // 查询购物车是否存在相同商品
                $cartInfo = $this->shop_cart_db->field('product_num')->where($where)->find();
                if (!empty($cartInfo)) {
                    // 购物车内已有相同产品，进行数量更新。
                    $data['product_num'] = $param['num'] + intval($cartInfo['product_num']); //与购物车数量进行叠加
                    $data['update_time'] = getTime();
                    $cart_id = $this->shop_cart_db->where($where)->update($data);
                } else {
                    // 购物车内还未有相同产品，进行添加。
                    ...
                    if (!empty($this->visitorsID) && empty($this->users_id)) $data['users_id'] = intval($this->visitorsID);
                    $cart_id = $this->shop_cart_db->add($data);
                }

                // 如果开启游客结账功能则执行
                $users_id = !empty($this->visitorsID) && empty($this->users_id) ? intval($this->visitorsID) : $this->users_id;
                // 购物车数量
                $cartNum = Db::name('shop_cart')->where(['users_id' => $users_id])->sum('product_num');

                if (!empty($cart_id)) {
                    $result = [
                        'code' => 1,
                        'cartNum' => intval($cartNum),
                        'cartHtml' => $this->fetch('./mini_cart'),
                    ];
                    $this->success(lang('users272', [], $this->home_lang), url('user/Shop/shop_cart_list'), $result);
                } else {
                    $this->error('加入购物车失败！', null, ['code' => -1]);
                }
            } else {
                $this->error(lang('users336', [], $this->home_lang), null, ['code'=>-1]);
            }
        } else {
    ...
    public function cart_unified_algorithm()
    {
        if (IS_AJAX_POST) {
            ...
            $spec_value_id = input('post.spec_value_id/s', '');
            ...
            $isCount = $this->archives_db->where($where)->count();
            if (!empty($isCount)) {
                // 查询条件
                $where = [
                    'users_id'    => $this->users_id,
                ...
                if ('-' == $symbol) $where['product_num'] = array('gt','1');
                $product_num = $this->shop_cart_db->where($where)->getField('product_num');
                // 处理购物车产品数量
                if (!empty($product_num)) {
                    // 更新数组
                    if ('+' == $symbol) {
                        $update['product_num'] = intval($product_num) + 1;
                    }
                    else if ('-' == $symbol) {
                        $update['product_num'] = intval($product_num) - 1;
                    }
                    ...
                    // 更新数据
                    $updateID = $this->shop_cart_db->where($where)->update($update);
                    ...
                    if (!empty($this->visitorsID) && empty($this->users_id)) $where['a.users_id'] = intval($this->visitorsID);
                    ...
                    $discountPrice = !empty($this->users['level_discount']) ? $this->users['level_discount'] / 100 : 1;
                    foreach ($cartData as $key => $value) {
                        if (!empty($value['spec_price'])) {
                            if (!empty($discountPrice)) $value['spec_price'] = $value['spec_price'] * $discountPrice;
                            $spec_price += $value['product_num'] * $value['spec_price'];
                        } else {
                            if (!empty($discountPrice)) $value['users_price'] = $value['users_price'] * $discountPrice;
                    ...
                    if (empty($cartData['num']) && empty($cartData['price'])) $cartData['price'] = $cartData['num'] = 0;
                    $where = [
                        'b.status' => 1,
                        'a.users_id' => $this->users_id,
                    ];
                    if (!empty($this->visitorsID) && empty($this->users_id)) $where['a.users_id'] = intval($this->visitorsID);
                    $cartAmountVal = $this->shop_cart_db->where($where)->alias('a')->join('__ARCHIVES__ b', 'a.product_id = b.aid', 'LEFT')->sum('product_num');
                    if (!empty($updateID)) {
                        $result = [
                        ...
                        $this->success(lang('sys19', [], $this->home_lang), null, $result);
                    }
                } else {
    ...
    public function cart_del()
    {
        if (IS_AJAX_POST) {
            ...
            $deteleAll = input('post.deteleAll/d', 0);
            if (!empty($cart_id) || !empty($deteleAll)) {
                // 删除条件
                $where = [
                    'cart_id'  => ['IN', explode(',', $cart_id)],
                    'users_id' => $this->users_id,
                ];
                if (!empty($deteleAll)) unset($where['cart_id']);
                if (!empty($this->visitorsID) && empty($this->users_id)) $where['users_id'] = intval($this->visitorsID);
                // 删除数据
                $resultID = $this->shop_cart_db->where($where)->delete(true);
            }
            if (!empty($resultID)) {
                // 计算购物车选中商品的总数总额
                $where = [
                ...
                $level_discount = !empty($this->users['level_discount']) ? $this->users['level_discount'] : 100;
                ...
                if (!empty($this->visitorsID) && empty($this->users_id)) $where['a.users_id'] = intval($this->visitorsID);
                // 购物车是否还存在商品
                $CartCount = $this->shop_cart_db->where($where)->alias('a')->join('__ARCHIVES__ b', 'a.product_id = b.aid', 'LEFT')->count();
                // 购物车所有商品数量
                $CartAmountVal = $this->shop_cart_db->where($where)->alias('a')->join('__ARCHIVES__ b', 'a.product_id = b.aid', 'LEFT')->sum('product_num');
                ...
                $this->success(lang('sys19', [], $this->home_lang), null, $data);
            } else {
                $this->error('删除失败！');
            }
        }
    ...
    public function shop_wechat_pay_select()
    {
        ...
        if (empty($ReturnOrderData)) {
            ...
            $this->error(lang('users337', [], $this->home_lang),$url);
        }
        $zan = [
    ...
    public function fastSubmitOrder()
    {
        ...
        if (IS_POST && !empty($aid) && !empty($pay_code)) {
            ...
            // 没有相应的产品
            if (empty($list[0])) $this->error('订单创建失败，没有相应的商品！');
    ...
    public function shop_payment_page()
    {
        if (IS_AJAX_POST) {
            // 提交的订单信息判断
            $post = input('post.');
            if (empty($post)) $this->error('订单创建失败，提交信息异常');
            ...
            if (!empty($Md5Value)) {
                ...
                // 商品数量判断
                if (intval($num) <= 0) $this->error('订单创建失败，购买数量不足1件');
                // 订单来源判断
                if (intval($type) != 1) $this->error('订单创建失败，提交信息来源异常');
                ...
                $field = 'a.aid, a.aid as product_id, a.title, d.title as title_new, a.litpic, a.users_price, a.users_discount_type, a.stock_count, a.prom_type, b.spec_price, b.spec_stock, b.auto_id, b.spec_value_id, a.free_shipping';
                // $field = 'a.aid, a.aid as product_id, a.title, a.litpic, a.users_price, a.users_discount_type, a.stock_count, a.prom_type, a.attrlist_id, b.spec_price, b.spec_stock, b.spec_value_id, b.value_id, c.spec_is_select, a.free_shipping';
                $list = $this->archives_db->field($field)
                    ->alias('a')
                    ->join('__PRODUCT_SPEC_VALUE__ b', 'a.aid = b.aid', 'LEFT')
                    // ->join('__PRODUCT_SPEC_DATA__ c', 'a.aid = c.aid and b.spec_value_id = c.spec_value_id', 'LEFT')
                    ->join('archives_'.$this->home_lang.' d', 'a.aid = d.aid', 'left')
                    ->where($where)
                    ->limit('0, 1')
                    ->select();
                ...
                } else if (!empty($post['cartidStr'])) {
                    $cartidArr = cookie($post['cartidStr']);
                    if (!empty($cartidArr)) {
                        unset($where['a.users_id']);
                        $where['a.cart_id'] = ['IN', $cartidArr];
                    }
                }
                $field = 'a.*, b.aid, b.title, d.title as title_new, b.litpic, b.users_price, b.users_discount_type, b.stock_count, b.prom_type, c.auto_id, c.spec_price, c.spec_stock, b.free_shipping';
                // $field = 'a.*, b.aid, b.title, b.litpic, b.users_price, b.users_discount_type, b.stock_count, b.prom_type, b.attrlist_id, c.spec_price, c.spec_stock, c.value_id, b.free_shipping';
                $list = $this->shop_cart_db->field($field)
                    ->alias('a')
                    ->join('__ARCHIVES__ b', 'a.product_id = b.aid', 'LEFT')
                    ->join('__PRODUCT_SPEC_VALUE__ c', 'a.spec_value_id = c.spec_value_id and a.product_id = c.aid', 'LEFT')
                    ->join('archives_'.$this->home_lang.' d', 'b.aid = d.aid', 'left')
                    ->where($where)
                    ->select();
            }

            // 没有相应的产品
            if (empty($list)) $this->error('订单创建失败，购买商品信息异常');
            ...
            if (empty($OrderData['prom_type']) && 1 === intval($OrderData['logistics_type'])) {
                ...
                /*if (empty($post['addr_id'])) {
                    ...
                    if (isWeixin() && !isWeixinApplets()) {
                        ...
                        $this->error('101:订单创建失败，请添加收货地址', null, $paramNew);
                    }
                }*/
                ...
                if (!empty($this->visitorsID) && empty($this->users_id)) $where['users_id'] = intval($this->visitorsID);
                $AddressData = $this->shop_address_db->where($where)->find();
                if (empty($AddressData)) {
                    if (isWeixin() && !isWeixinApplets()) {
                        ...
                        $this->error('102:订单创建失败，请添加收货地址', null, $paramNew);
                    }
                }*/
                ...
                if (!empty($this->visitorsID)) {
                    if (empty($post['email'])) {
                        $this->error(lang('users114', [], $this->home_lang), null, ['field' => 'email']);
                    } else if (!check_email($post['email'])) {
                        $this->error(lang('users121', [], $this->home_lang), null, ['field' => 'email']);
                    }
                    $post['email'] = trim($post['email']);
                    $post['users_id'] = intval($this->visitorsID);
                }

                if (empty($post['consignee'])) {
                    $users277 = lang('users277', [], $this->home_lang);
                    $this->error(lang('users271', [$users277], $this->home_lang), null, ['field' => 'consignee']);
                }
                else if (empty($post['address'])) {
                    $users229 = lang('users229', [], $this->home_lang);
                    $this->error(lang('users271', [$users229], $this->home_lang), null, ['field' => 'address']);
                }
                else if (empty($post['city_name'])) {
                    $users226 = lang('users226', [], $this->home_lang);
                    $this->error(lang('users271', [$users226], $this->home_lang), null, ['field' => 'city_name']);
                }
                else if (empty($post['province_name'])) {
                    $users228 = lang('users228', [], $this->home_lang);
                    $this->error(lang('users271', [$users228], $this->home_lang), null, ['field' => 'province_name']);
                }
                else if (empty($post['country'])) {
                    $users227 = lang('users227', [], $this->home_lang);
                    $this->error(lang('users271', [$users227], $this->home_lang), null, ['field' => 'country']);
                }
                else if (empty($post['mobile'])) {
                    $users224 = lang('users224', [], $this->home_lang);
                    $this->error(lang('users271', [$users224], $this->home_lang), null, ['field' => 'mobile']);
                }
                else if (empty($post['zipcode'])) {
                    $users225 = lang('users225', [], $this->home_lang);
                    $this->error(lang('users271', [$users225], $this->home_lang), null, ['field' => 'zipcode']);
                }

                // 运费处理
                $shipping_fee = 0;
                if (!empty($this->usersConfig['shop_open_shipping'])) {
                    // 固定运费
                    if (!empty($this->usersConfig['shop_open_shipping_type']) && 1 === intval($this->usersConfig['shop_open_shipping_type'])) {
                        $shipping_fee = unifyPriceHandle($this->usersConfig['shop_open_shipping_money']);
                    }
                    // 百分比计算运费
                    else if (!empty($this->usersConfig['shop_open_shipping_type']) && 2 === intval($this->usersConfig['shop_open_shipping_type'])) {
                        
                    }
                }
                if (!empty($shipping_fee)) {
                    // 合计金额加上运费价格
                    $OrderData['order_amount'] += $shipping_fee;
                    $OrderData['order_total_amount'] += $shipping_fee;
                }
                ...
                if (!empty($this->visitorsID) && empty($this->users_id)) {
                    $where['users_id'] = intval($this->visitorsID);
                }
                else if (!empty($post['addr_id']) && !empty($this->users_id)) {
                    $where = array_merge(['addr_id' => intval($post['addr_id'])], $where);
                }
                $usersAddr = $this->shop_address_db->where($where)->find();
                if (empty($usersAddr)) {
                    if (isset($where['addr_id'])) unset($where['addr_id']);
                    ...
                    if (!empty($this->visitorsID) && !empty($post['email']) && empty($this->users_id)) $insert['email'] = trim($post['email']);
                    $addr_id = $this->shop_address_db->insertGetId($insert);
                    if (!empty($addr_id)) $usersAddr = $this->shop_address_db->where(['addr_id' => intval($addr_id)])->find();
                }

                // 拼装数组
                $orderAddr = [
                    'consignee'    => $usersAddr['consignee'],
                    'country'      => $usersAddr['country'],
                    'province'     => $usersAddr['province'],
                    'province_name'=> $usersAddr['province_name'],
                    'city'         => $usersAddr['city'],
                    'city_name'    => $usersAddr['city_name'],
                    'district'     => $usersAddr['district'],
                    'district_name'=> $usersAddr['district_name'],
                    'address'      => $usersAddr['address'],
                    'mobile'       => $usersAddr['mobile'],
                    'email'        => '',
                    'zipcode'      => $usersAddr['zipcode'],
                    'shipping_fee' => $shipping_fee,
                ];
                if (!empty($this->visitorsID) && !empty($post['email']) && empty($this->users_id)) $orderAddr['email'] = trim($post['email']);
            }

            // 存在收货地址则追加合并到主表数组
            if (!empty($orderAddr)) $OrderData = array_merge($OrderData, $orderAddr);

            $payApiConfig = [];
            if (0 < $OrderData['order_amount']) {
                if (!empty($post['payment_type'])) {
                    $paymentTypeArr = explode('_', $post['payment_type']);
                    $payMark = !empty($paymentTypeArr[1]) ? trim($paymentTypeArr[1]) : '';
                    $payApiConfig = Db::name('pay_api_config')->where(['pay_mark' => $payMark])->find();
                    $payApiConfig['pay_info'] = !empty($payApiConfig['pay_info']) ? unserialize($payApiConfig['pay_info']) : [];
                    if (empty($payApiConfig) || empty($payApiConfig['pay_info']['business'])) $this->error(lang('users371', [], $this->home_lang));
                } else {
                    $this->error(lang('users371', [], $this->home_lang));
                }
                // 数据验证
                $rule = ['payment_type' => 'require|token'];
                $message = ['payment_type.require' => '不可为空'];
                $validate = new \think\Validate($rule, $message);
                if (!$validate->check($post)) $this->error(lang('users372', [], $this->home_lang));
            }
            if (!empty($payApiConfig['pay_mark'])) $OrderData['pay_name'] = trim($payApiConfig['pay_mark']);
            ...
            if (!empty($OrderId)) {
                ...
                // 添加订单明细表
                $DetailsId = $this->shop_order_details_db->insertAll($retData['data']['orderDetails']);
                if (!empty($DetailsId)) {
                    // 清理购物车中已下单的ID
                    if (!empty($retData['data']['cart_ids'])) $this->shop_cart_db->where('cart_id', 'IN', $retData['data']['cart_ids'])->delete(true);

                    // 产品库存、销量处理
                    if (!empty($retData['data']['upSpecValue'])) $this->shop_model->ProductStockProcessing($retData['data']['upSpecValue']);

                    // 添加订单操作记录
                    AddOrderAction($OrderId, !empty($this->visitorsID) && empty($this->users_id) ? intval($this->visitorsID) : intval($this->users_id));
                    ...
                    // cookie($paystr, $paydata);
                    // // 跳转链接
                    // $this->success(lang('users291', [], $this->home_lang), urldecode(dynamic_url('user/Shop/shop_payment_page', ['paystr' => $paystr])));
                    ...
                    $this->success(lang('users291', [], $this->home_lang), null, $result);
                    ...
                    // 删除新增的订单
                    $this->shop_order_db->where(['order_id' => intval($OrderId)])->delete(true);
                    $this->error('订单创建失败，请刷新页面再次下单');
                }
            } else {
                $this->error('订单创建失败，请刷新页面再次下单');
            }
        ...
        // 订单信息字符串
        $paystr = input('paystr/s', '');
        $orderArr = !empty($paystr) ? cookie($paystr) : [];
        if (empty($orderArr['order_id']) || empty($orderArr['order_code'])) $this->error('订单提交异常，请重新提交');

        // 查询订单信息
        $where = [
            'users_id' => $this->users_id,
            'order_id' => intval($orderArr['order_id']),
            'order_code' => trim($orderArr['order_code']),
        ];
        if (!empty($this->visitorsID) && empty($this->users_id)) $where['users_id'] = intval($this->visitorsID);
        $orderData = $this->shop_order_db->where($where)->find();
        if (empty($orderData)) $this->error('订单提交异常，请重新提交');
        $this->assign('orderData', $orderData);

        return $this->fetch('users/shop_payment_page');
    }

    public function shop_pay_success()
    {
        // 跳转地址
        $url = custom_url('user/Shop/shop_centre');
        if (!empty($this->visitorsID)) {
            // 游客订单查询不到则跳转到首页
        ...
        if (empty($order_id)) $this->error(lang('users332', [], $this->home_lang), $url);
        ...
        if (!empty($this->users_id)) {
            $where['users_id'] = intval($this->users_id);
        }
        else if (!empty($this->visitorsID)) {
            $where['users_id'] = intval($this->visitorsID);
        }
        $orderData = $this->shop_order_db->where($where)->find();
        if (empty($orderData)) $this->error(lang('users332', [], $this->home_lang), $url);
        if (!empty($orderData['pay_time'])) $orderData['pay_time'] = date('Y-m-d H:i:s', $orderData['pay_time']);
        $this->assign('orderData', $orderData);

        return $this->fetch('users/shop_pay_success');
    }
    ...
    public function to_split_goods()
    {
        ...
        $field = 'a.*, b.aid, b.title, b.litpic, b.users_price, b.logistics_type, b.users_discount_type, b.stock_count, b.is_del, c.spec_price, c.spec_stock';
    ...
    public function shop_add_address()
    {
        if (IS_AJAX_POST) {
            $post = input('post.');
            // if (empty($post['consignee'])) $this->error('请填写姓名');
            // if (empty($post['mobile'])) $this->error('请填写电话号码');
            // if (empty($post['zipcode'])) $this->error('请填写邮编');
            // if (empty($post['city_name'])) $this->error('请填写城市');
            // if (empty($post['country'])) $this->error('请选择国家/地区');
            // if (empty($post['province_name'])) $this->error('请填写省份');
            // if (empty($post['address'])) $this->error('请填写详细地址');

            if (empty($post['consignee'])) {
                $users208 = lang('users208', [], $this->home_lang);
                $this->error(lang('users271', [$users208], $this->home_lang));
            }
            if (empty($post['mobile'])) {
                $users224 = lang('users224', [], $this->home_lang);
                $this->error(lang('users271', [$users224], $this->home_lang));
            }
            if (empty($post['zipcode'])) {
                $users225 = lang('users225', [], $this->home_lang);
                $this->error(lang('users271', [$users225], $this->home_lang));
            }
            if (empty($post['city_name'])) {
                $users226 = lang('users226', [], $this->home_lang);
                $this->error(lang('users271', [$users226], $this->home_lang));
            }
            if (empty($post['country'])) {
                $users227 = lang('users227', [], $this->home_lang);
                $this->error(lang('users271', [$users227], $this->home_lang));
            }
            if (empty($post['province_name'])) {
                $users228 = lang('users228', [], $this->home_lang);
                $this->error(lang('users271', [$users228], $this->home_lang));
            }
            if (empty($post['address'])) {
                $users229 = lang('users229', [], $this->home_lang);
                $this->error(lang('users271', [$users229], $this->home_lang));
            }

            // 添加数据
            $post['consignee'] = trim($post['consignee']);
            $post['mobile'] = intval($post['mobile']);
            $post['zipcode'] = trim($post['zipcode']);
            $post['city_name'] = trim($post['city_name']);
            $post['country'] = intval($post['country']);
            $post['province_name'] = trim($post['province_name']);
            $post['address'] = trim($post['address']);
            $post['users_id'] = intval($this->users_id);
            ...
            if (!empty($addr_id)) {
                // 把对应会员下的所有地址改为非默认
                if (!empty($post['is_default']) && !empty($this->users_id)) {
                    $where = [
                ...
                if ((isMobile() || isWeixin()) && !empty($post['v2x'])) {
                    $url = session($this->users_id.'_EyouShopOrderUrl');
                    $this->success(lang('users272', [], $this->home_lang), $url, ['url' => $url]);
                }

                $types = input('types/s', input('type/s', ''));
                if (!empty($types) && 'list' === trim($types)) {
                    $url = url('user/Shop/shop_address_list');
                    $this->success(lang('users272', [], $this->home_lang), $url, ['url' => $url]);
                }
                else if (!empty($types) && 'order' === trim($types)) {
                    $this->success(lang('users272', [], $this->home_lang), dynamic_url('user/Shop/shop_under_order', ['addr_id' => $addr_id, 'querystr' => input('querystr/s', '')]));
                }
        ...
        if ('list' == $types || 'order' == $types || 'order_new' == $types) {
            ...
            $zan = [
                ...
                'countryList' => Db::name('country')->where('status', 1)->field('id, name')->order('sort_order asc, id desc')->select(),
            ];
            $this->zan = array_merge($this->zan, $zan);
            $this->assign('zan', $this->zan);
        } else {
        ...
        // 下单加密串
        $this->assign('querystr', input('querystr/s', ''));
    ...
    public function shop_edit_address()
    {
        if (IS_AJAX_POST) {
            ...
            if (!empty($addr_id)) {
                ...
                if (isset($post['is_default']) && 1 == $post['is_default']) {
                    ...
                    $this->shop_address_db->where($where)->update($data_new);
                }

                // 如果是移动端(手机端、微信端)并且有v2x参数则执行
                if ((isMobile() || isWeixin()) && !empty($post['v2x'])) {
                    $url = session($this->users_id.'_EyouShopOrderUrl');
                    $this->success(lang('users237', [], $this->home_lang), $url, ['url' => $url]);
                }

                $post['country_name'] = Db::name('country')->where(['id' => intval($post['country'])])->getField('name');
                // $post['province'] = get_province_name($post['province']);
                // $post['city']     = get_city_name($post['city']);
                // $post['district'] = get_area_name($post['district']);
                $this->success(lang('users237', [], $this->home_lang), null, $post);
            } else {
                $this->error('数据有误！');
            }
        }
        ...
        $address['types'] = trim($type);
        $gourl = input('param.gourl/s', '', 'urldecode');
        $address['delAddressUrl'] = dynamic_url('Shop/shop_del_address', ['_ajax' => 1, 'type' => $type, 'gourl' => $gourl]);

        // 国家表
        $countryList = Db::name('country')->where('status', 1)->field('id, name')->order('sort_order asc, id desc')->getAllWithIndex('id');
        // 国家地址名称
        $address['country_name'] = !empty($countryList[$address['country']]['name']) ? trim($countryList[$address['country']]['name']) : '未知';
    ...
    public function shop_del_address()
    {
        if (IS_POST) {
            ...
            $deleteID = $this->shop_address_db->where($where)->delete(true);
            if (!empty($deleteID)) {
                $gourl = '';
                $type = input('param.type/s', 'list');
                if ('order' == $type) $gourl = input('param.gourl/s', '', 'urldecode');
                $this->success(lang('users273', [], $this->home_lang), null, ['url' => url('Shop/shop_address_list', ['type' => $type, 'gourl' => $gourl])]);
            } else {
                $this->error(lang('users277', [], $this->home_lang));
            }
        }
    }
    ...
    public function shop_inquiry_shipping()
    {
        if (IS_AJAX_POST) {
            ...
            if (!empty($querystr)) {
                ...
                $archives = $this->archives_db->field('free_shipping')->where($where)->select();
            } else {
                $where = [
                    'a.selected' => 1,
                    'a.users_id' => $this->users_id,
                ];
                $field = 'b.free_shipping';
    ...
    public function shop_order_remind()
    {
        if (IS_AJAX_POST) {
            ...
            $this->success(lang('users330', [], $this->home_lang)
);
        }else{
            $this->error('订单号错误');
        }
    ...
    private function IsSoldOut($param = array())
    {
       if (!empty($param['aid'])) {
            if (!empty($param['spec_value_id'])) {
                ...
                $spec_stock = Db::name('product_spec_value')->where($where)->getField('spec_stock');
                if (empty($spec_stock)) {
                    $data['code'] = -1; // 已售罄
                    $this->error(lang('users338', [], $this->home_lang), null, $data);
                }
                ...
                $stock_count = $this->archives_db->where($where)->getField('stock_count');
    ...
    public function service_list()
    {
        ...
        if (empty($order_id)) $this->error('订单不11存在！');
        // 查询订单商品
        $where = [
            'order_id' => $order_id,
            'users_id' => $this->users_id,
        ...
        if (empty($order_details)) $this->error('订单22不存在！');
        // 查询订单商品售后
