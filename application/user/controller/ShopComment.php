...
class ShopComment extends Base
{
    ...
    public function need_comment_list()
    {
        ...
        if (!empty($res)) {
            $where = [
                'is_comment' => 0,
                'order_id' => ['IN', array_unique(get_arr_column($res, 'order_id'))],
            ];
            $field = 'product_id, product_name, litpic, order_id';
            $goodsArr = Db::name('shop_order_details')->field($field)->where($where)->select();
            if (!empty($goodsArr)) {
                $archives = Db::name('archives')->where(['aid' => ['IN', array_unique(get_arr_column($goodsArr, 'product_id'))]])->getAllWithIndex('aid');
                foreach ($goodsArr as $key => $value) {
                    if (!empty($archives[$value['product_id']])) {
                        $goodsArr[$key] = array_merge($value, $archives[$value['product_id']]);
                    } else {
                        // $value['aid'] = $value['product_id'];
            ...
            // dump($goodsArr);exit;
            $archivesNew = get_archives_data($goodsArr, 'product_id');
            foreach ($goodsArr as $k=> $v) {
                $v['arcurl'] = '';
                if (!empty($archivesNew[$v['product_id']]) && !empty($archivesNew[$v['product_id']]['htmlfilename'])) {
                    $v['arcurl'] = urldecode(arcurl('home/Product/view', $archivesNew[$v['product_id']]));
                }
    ...
    public function add_comment()
    {
        if (IS_AJAX_POST) {
            ...
            if (empty($post['total_score'])) $this->error(lang('users302', [], $this->home_lang));
            $count = $content = 0;
            foreach ($post['total_score'] as $key => $value) {
                if (!empty($value)) $count++;
                if (!empty($post['content'][$key])) $content++;
            }
            if (0 === intval($count) || intval($count) !== intval(count($post['details_id']))) $this->error(lang('users302', [], $this->home_lang));
            if (0 === intval($content) || intval($content) !== intval(count($post['details_id']))) $this->error(lang('users303', [], $this->home_lang));
            ...
            if (!empty($resultID)) $this->error(lang('users304', [], $this->home_lang));
            ...
            if (!empty($resultID)) {
                ...
                // 返回提示结束
                $this->success(lang('users305', [], $this->home_lang), in_array($this->usersTplVersion, ['v3']) ? url('user/ShopComment/need_comment_list') : url('user/ShopComment/index'));
            } else {
