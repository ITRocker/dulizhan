...
class Buildhtml extends Base
{
    ...
    private function buildAppointAll($uphtmltype = 0)
    {
        if (1 == $uphtmltype) { // 指定时间的文档更新
            $seo_start_time = $this->zan['global']['seo_start_time'];
            ...
            $startid = $this->zan['global']['seo_startid2'];
        }
    ...
    public function buildIndex()
    {
        ...
        if (isset($param['seo_showmod'])) { // 是否要保存生成静态的配置
            $this->zan['global']['seo_showmod'] = $param['seo_showmod'];
            /*多语言*/
    ...
    private function handleBuildIndex()
    {
        ...
        // 保存的文件名
        $seo_html_position_arr = explode('/', $this->zan['global']['seo_html_position']);
        ...
        if (!empty($this->zan['global']['seo_showmod'])){
            $seo_html_position   = !empty($this->zan['global']['seo_html_position']) ? $this->zan['global']['seo_html_position'] : '';
            ...
            try {
                ...
                $tpl      = 'index';
                $seo_html_templet   = !empty($this->zan['global']['seo_html_templet']) ? $this->zan['global']['seo_html_templet'] : '';
    ...
    private function filePutContents($savepath, $tpl, $model = 'pc', $pages = 0, $dir = '/', $tid = 0, $top = 1, $result = [])
    {
        ...
        if ($pages > 0) {
            ...
            $dir = trim($dir, '.');
            $seo_html_listname = $this->zan['global']['seo_html_listname'];
    ...
    {
        !empty($this->zan['global']['seo_pagesize']) && $limit = $this->zan['global']['seo_pagesize'];
        $startid  = 0;
        $endid  = 0;
        $uphtmltype = input('param.uphtmltype/d');
        if (!empty($uphtmltype)) { // 更新选项/指定时间或者文档ID，最后都是转为以文档起始ID
            $startid = input('param.startid/d');
            empty($startid) && $startid  = !empty($this->zan['global']['seo_startid']) ? intval($this->zan['global']['seo_startid']) : 0;
            $endid  = !empty($this->zan['global']['seo_endid']) ? intval($this->zan['global']['seo_endid']) : 0;
        }
        $msg                  = "";
        $globalConfig         = $this->zan['global'];
    ...
    private function createArticle($result, $globalConfig, $arctypeRow, $allTags, $has_children_Row, $allAttrInfo)
    {
        ...
        if (4 == $this->zan['global']['seo_html_pagename']) {
...
//            $seo_upnext = !empty($this->zan['global']['seo_upnext']) ? true : false; // 是否更新子栏目
//            foreach ($info as $k => $v) {
//                if (empty($seo_upnext)) {
...
//            $info = array_values($info);//重组数组
    ...
    private function handleBuildChannelList($id, $findex = 0, $achievepage = 0, $batch = true, $parent = true,$index = 0, $limit = 50){
        !empty($this->zan['global']['seo_maxpagesize']) && $limit = $this->zan['global']['seo_maxpagesize'];
        $msg                  = '';
    ...
    private function createChannelList($row, $has_children_Row,$index = 0, $limit = 10)
    {
        ...
        $seo_html_listname = $this->zan['global']['seo_html_listname'];
        $seo_html_arcdir   = $this->zan['global']['seo_html_arcdir'];
        $tid               = $row['typeid'];
        ...
        if (in_array($row['current_channel'], [6, 8])) {   //留言模型或单页模型，不存在多页
            ...
            $dirpath     = explode('/', $zan['field']['dirpath']);
            $dirpath_end = end($dirpath);
            if ($seo_html_listname == 1) {  //存放顶级目录
                $savepath = '.' . $seo_html_arcdir . '/' . $dirpath[1] . "/lists_" . $zan['field']['typeid'] . ".html";
            } else if ($seo_html_listname == 3) { //存放子级目录
                $savepath = '.' . $seo_html_arcdir . '/' . $dirpath_end . "/lists_" . $zan['field']['typeid'] . ".html";
            } else if ($seo_html_listname == 4) { //自定义存放目录
                $savepath = '.' . $seo_html_arcdir;
                $diy_dirpath = !empty($zan['field']['diy_dirpath']) ? $zan['field']['diy_dirpath'] : '';
                if (!empty($zan['field']['rulelist'])) {
                    $rulelist = ltrim($zan['field']['rulelist'], '/');
                    $rulelist = str_replace("{tid}", $zan['field']['typeid'], $rulelist);
                    ...
                    $zan['field']['rulelist'] = '{栏目目录}/list_{tid}_{page}.html';
                    $savepath .= $diy_dirpath . '/' . 'list_' . $zan['field']['typeid'] . ".html";
                }
            } else {
                $savepath = '.' . $seo_html_arcdir . $zan['field']['dirpath'] . '/' . 'lists_' . $zan['field']['typeid'] . ".html";
            }
            try {
                ...
                if ($seo_html_listname == 3) {
                    ...
                    if (preg_match('/^{(栏目目录|typedir)}\/list_{tid}_{page}\.html$/i', $zan['field']['rulelist'])) {
                        $dst_savepath = preg_replace('/\/([^\/]+)$/i', '/index.html', $savepath);
                        @copy($savepath, $dst_savepath);
                        @unlink($savepath);
                    }
                } else if ($seo_html_listname == 2 || count($dirpath) < 3) {
                    @copy($savepath, '.' . $seo_html_arcdir . $zan['field']['dirpath'] . '/index.html');
                    @unlink($savepath);
                }
            } catch (\Exception $e) {
                $msg .= '<span>' . $savepath . '生成失败！' . $e->getMessage() . '</span><br>';
        ...
        if (in_array($row['current_channel'], [6, 8])) {   //留言模型或单页模型，不存在多页
            ...
            if ($seo_html_listname == 1) {  //存放顶级目录
                ...
                if (!empty($zan['field']['rulelist'])) {
                    ...
                    $zan['field']['rulelist'] = '{栏目目录}/lists_{tid}_{page}.html';
                    $savepath .= $diy_dirpath . '/' . 'list_' . $zan['field']['typeid'] . ".html";
                }
            } else {
                $savepath = '.' . $seo_html_arcdir . $zan['field']['dirpath'] . '/' . 'lists_' . $zan['field']['typeid'] . ".html";
    ...
    private function createMultipageChannel($i, $tid, $row, $has_children_Row, $seo_html_listname, $seo_html_arcdir, $tpl)
    {
        ...
        $row['seo_title'] = set_typeseotitle($row['typename'], $row['seo_title_tmp'], $this->zan['site']);
        // $row        = $this->lists_logic($row, $has_children_Row);  // 模型对应逻辑
        ...
        if ($seo_html_listname == 1) {  //存放顶级目录
            ...
            $savepath = '.' . $seo_html_arcdir . '/' . $dirpath[1] . "/lists_" . $zan['field']['typeid'];
        } else if ($seo_html_listname == 3) { //存放子级目录
            $dir      = '.' . $seo_html_arcdir . '/' . $dirpath_end;
            $savepath = '.' . $seo_html_arcdir . '/' . $dirpath_end . "/lists_" . $zan['field']['typeid'];
        } else if ($seo_html_listname == 4) { //自定义存放目录
            ...
            if (!empty($zan['field']['rulelist'])) {
                ...
                $dir .= $diy_dirpath;
                $savepath .= $diy_dirpath . '/' . 'list_' . $zan['field']['typeid'];
            }
        } else {
            $dir      = '.' . $seo_html_arcdir . $zan['field']['dirpath'];
            $savepath = '.' . $seo_html_arcdir . $zan['field']['dirpath'] . '/' . 'lists_' . $zan['field']['typeid'];
        }

        if ($seo_html_listname != 4 || empty($zan['field']['rulelist'])) {
            if ($i > 1) {
                $savepath .= '_' . $i . '.html';
            } else {
                $savepath .= '.html';
        ...
        try {
            ...
            if ($i == 1 && $seo_html_listname == 3) {
                ...
                if ($i == 1) {
                    ...
                    if (!empty($zan['field']['rulelist']) && !preg_match('/{page}/i', $zan['field']['rulelist'])) { // 没有分页变量的情况
                        @unlink($savepath);
                    }
                }
    ...
    public function upHtml()
    {
        ...
        $seo_pseudo = $this->zan['global']['seo_pseudo'];
        $seo_html_pagename = $this->zan['global']['seo_html_pagename'];
        $this->clearCache();
        if ($seo_pseudo != 2) {
        ...
        if (!empty($del_ids)) {    //删除文章页面
            $this->buildhtmlLogic->delViewHtml($del_ids, $this->zan['global']);
    ...
    private function lists_logic($result = [], $has_children_Row = [])
    {
        ...
        $result['seo_title'] = set_typeseotitle($result['typename'], $result['seo_title_tmp'], $this->zan['site']);
    ...
    private function pc_to_mobile_js($html = '', $result = [])
    {
        ...
        if (file_exists('./template/'.TPL_THEME.'mobile')) { // 分离式模板
            ...
            /*是否开启手机站域名，并且配置*/
            if (!empty($this->zan['global']['web_mobile_domain_open']) && !empty($this->zan['global']['web_mobile_domain'])) {
                $domain = $this->zan['global']['web_mobile_domain'] . '.' . $this->request->rootDomain();
            } else {
                $domain = true;
            }
            /*end*/
...
EOF;
            ...
            if (!empty($this->zan['global']['web_mobile_domain_open']) && !empty($this->zan['global']['web_mobile_domain'])) {
                if (empty($result['pageurl'])) {
                    $url = $this->request->subDomain($this->zan['global']['web_mobile_domain']) . ROOT_DIR . '/index.php';
                } else {
                    $url = !preg_match('/^(http(s?):)?\/\/(.*)$/i', $result['pageurl']) ? $this->request->domain() . $result['pageurl'] : $result['pageurl'];
                    $url = preg_replace('/^(.*)(\/\/)([^\/]*)(\.?)(' . $this->request->rootDomain() . ')(.*)$/i', '${1}${2}' . $this->zan['global']['web_mobile_domain'] . '.${5}${6}', $url);
                }

                $mobileDomain = $this->zan['global']['web_mobile_domain'] . '.' . $this->request->rootDomain();
                $jsStr        = <<<EOF
    <meta http-equiv="mobile-agent" content="format=xhtml;url={$url}">
