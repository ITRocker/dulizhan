...
if (!function_exists('zanSetting')) 
{
    /**
     * 不分语言的标记表 - 获取缓存或者更新缓存，只适用于setting_syn表
     * @param string $config_key 缓存文件名称
     * @param array $data 缓存数据  array('k1'=>'v1','k2'=>'v3')
     * @param array $options 缓存配置
     * @return array or string or bool
     */
    function zanSetting($config_key, $data = array(), $options = null){
        $tableName = 'setting_syn';
        $table_db = \think\Db::name($tableName);

        $param = explode('.', $config_key);
        $cache_inc_type = "{$tableName}-{$param[0]}";
        if (empty($options['path'])) {
            $options['path'] = DATA_PATH.'runtime'.DS.'cache'.DS;
        }
        if(empty($data)){
            //如$config_key=shop_info则获取网站信息数组
            //如$config_key=shop_info.logo则获取网站logo字符串
...
if (!function_exists('write_global_params')) 
{
     ...
     * @return array
     */
    function write_global_params($lang = '', $options = null, $synParam = [])
    {
        $inc_type = 'web';
        ...
        $param['web_eyoucms'] = str_replace('#', '', '#h#t#t#p#:#/#/#w#w#w#.#z#a#n##c#m#s#.#c#o#m#'); // eyou网址

        foreach ($param as $key => $val) {
            if (isset($synParam[$key])) {
                $param[$key] = $synParam[$key];
            }
        }
...
if (!function_exists('write_html_cache')) 
{
    ...
    function write_html_cache($html = '', $result = []){
        ...
        if ($html_cache_status && !empty($html_cache_arr) && !empty($html)) {
            ...
            // aid唯一性的处理
            if (isset($param['aid'])) {
                if (!preg_match("/^([\x{4e00}-\x{9fa5}\w\-]+)$/u", $param['aid'])) {
                    abort(404,'页面不存在');
                }
            }
            ...
            foreach($html_cache_arr as $mca=>$val)
            {
                    ...
                    foreach ($val['p'] as $k=>$v) {
                        if (isset($param[$v])) {
                            if (preg_match('/\/$/i', $filename)) {
                                ...
                                $filename .= in_array($v, ['aid']) ? md5($param[$v]) : $param[$v];
                            }
                        }
                    }
                    !empty($tid) && $cache_tag = $tid; // 针对列表缓存的标签
                    !empty($param['aid']) && $cache_tag = md5($param['aid']); // 针对内容缓存的标签
                }
                ...
                if (1 == intval($web_cmsmode)) { // 永久
                    ...
                    $new_filename = TCP_SCHEME.'_'.$home_lang;
                    ...
                    $filename = preg_replace("/([^\x{4e00}-\x{9fa5}\w\-]+)/u", '', $new_filename);
...
if (!function_exists('ui_read_bidden_inc')) {
     ...
     * @return mixed
     */
    function ui_read_bidden_inc($filenameArr)
    {
        $num = 0;
        $dataArr = [];
        $page = '';
        ...
        foreach ($filenameArr as $key => $filename) {
            ...
            if ($num == 0) {
                ...
                $cacheKey = "application-common-ui_read_bidden_inc-{$page}";
                $ui_config_list = \think\Cache::get($cacheKey);
                if (empty($ui_config_list)) {
                    $map = array(
                        'page'   => $page,
                        'theme_style'   => THEME_STYLE_PATH,
                    );
                    $row = M('ui_config')->where($map)->select();
                    if (!empty($row)) {
                        foreach ($row as $k => $v) {
                            $ui_config_list[$v['lang']][$v['page']]["{$v['lang']}_{$v['type']}_{$v['name']}"] = $v;
                        }
                    }
                    \think\Cache::set($cacheKey, $ui_config_list, EYOUCMS_CACHE_TIME, "ui_config");
                }
            }

            if (!empty($data)) {
                if (empty($ui_config_list[$key][$page])) {
                    $data = [];
                } else {
                    $ui_config_keys = array_keys($ui_config_list[$key][$page]);
                    $data_keys = array_keys($data);
                    $diff_keys = array_diff($ui_config_keys, $data_keys);
                    if (!empty($diff_keys)) {
                        $data = [];
                    }
                }
            ...
            if (empty($data)) {
                // -------------优先读取配置文件，不存在才读取数据表
                if ($ui_config_list[$key][$page]) {
                    $arr = array();
                    foreach ($ui_config_list[$key][$page] as $_k => $_v) {
                        $k = "{$_v['lang']}_{$_v['type']}_{$_v['name']}";
                        $arr[$k] = $_v['value'];
                    }
                    $data = $arr;
                } else {
                ...
                if (!empty($data)) {
                    ...
                    tp_mkdir(dirname($filename));
                    $fp = @fopen($filename, "w+");
                    if (is_writeable($filename) && !empty($fp)) {
                        $nowData = $data;
                        $setting = "<?php die('forbidden'); ?>\n";
                        $setting .= json_encode($nowData);
                        $setting = str_replace("\/", "/",$setting);
                        if (@fwrite($fp, $setting)) {
                            fclose($fp);
                        } else {
                            @file_put_contents($filename, $setting);
                        }
                    }
                    //---------------end
                }
...
if (!function_exists('ui_write_bidden_inc')) {
     ...
     * @return mixed
     */
    function ui_write_bidden_inc($dataArr = [], $filenameArr = [], $is_append = false)
    {
        $dataArr2 = $dataArr;
        if (!empty($filenameArr)) {
            ...
            $ui_config_list = [];
            $addData = $editData = [];
            foreach ($dataArr2 as $key2 => $data2) {
                reset($data2);
                $value = current($data2);
                ...
                $theme_style = THEME_STYLE_PATH;
                $md5key = md5($name.$page.$theme_style);
                // 仅第一次执行
                if ($num == 0) {
                    $map = array(
                        'name'   => $name,
                    ...
                    $row = M('ui_config')->field('id,md5key,lang')->where($map)->order('lang asc')->select();
                    if (!empty($row)) {
                        foreach ($row as $_k => $_v) {
                            $ui_config_list[$_v['lang']][$_v['md5key']] = $_v;
                        }
                    }
                }
                ...
                $ui_config_info = empty($ui_config_list[$lang][$md5key]) ? [] : $ui_config_list[$lang][$md5key];
                if (!empty($ui_config_info)) {
                    $savedata['id'] = $ui_config_info['id'];
            ...
            $r = true;
            if (!empty($addData)) {
                $r = model('UiConfig')->saveAll($addData);
            }
            ...
            if ($r !== false) {
                ...
                $del_ids = [];
                foreach ($r as $k1 => $v1) {
                    $del_ids[] = $v1->getData('id');
                }
                if (!empty($del_ids)) {
                    \think\Db::name('ui_config')->where(['id'=>['IN', $del_ids]])->delete(true);
            ...
            if ($r !== false) {
                ...
                // 读取文件缓存
                $incArr = ui_read_bidden_inc($filenameArr);
                // ----------同时写入文件缓存
                foreach ($filenameArr as $key => $filename) {
                    tp_mkdir(dirname($filename));
                    $is_writeable_status = 0;
                    $fp = @fopen($filename, "w+");
                    if (is_writeable($filename) && !empty($fp)) {
                        // 追加
                        if ($is_append) {
                            $inc = empty($incArr[$key]) ? [] : (array)$incArr[$key];
                            if ($inc) {
                                $oldarr = $inc;
                                $dataArr[$key] = array_merge($oldarr, $dataArr[$key]);
                            }
                        }
                        $setting = "<?php die('forbidden'); ?>\n";
                        $setting .= json_encode($dataArr[$key]);
                        $setting = str_replace("\/", "/", $setting);
                        if (@fwrite($fp, $setting)) {
                            $is_writeable_status = 1;
                            fclose($fp);
                        } else {
                            if (@file_put_contents($filename, $setting)) {
                                $is_writeable_status = 1;
                            }
                        }
                    }
                    if (empty($is_writeable_status)) {
                        return ['code'=>0, 'msg'=>'data目录没有读写权限'];
                    }
                }
                //---------------end
...
if (!function_exists('get_ui_inc_params')) {
     ...
     * @return mixed
     */
    function get_ui_inc_params($page, $lang)
    {
        $e_page = $page;
        $filenameArr = [];
        $filenameArr[$lang] = RUNTIME_PATH."ui/".THEME_STYLE_PATH."/{$lang}/".$e_page.".inc.php";
        $inc = ui_read_bidden_inc($filenameArr);

        return empty($inc[$lang]) ? [] : $inc[$lang];
    }
}
...
if (!function_exists('get_main_lang')) 
{
     ...
     * 获取主体语言（语言列表里默认的一条）
     */
    function get_main_lang($is_force = false, $is_static = true)
    {
        if ($is_static) {
...
if (!function_exists('get_default_lang')) 
{
     ...
     * 获取默认语言
     */
    function get_default_lang($is_force = false, $is_static = true)
    {
        if ($is_static) {
        ...
        if (null === $default_lang || $is_force) {
            $request = \think\Request::instance();
            if (!stristr($request->baseFile(), 'index.php')) {
                $default_lang = \think\Db::name('language')->where([
                    'is_admin_default'   => 1,
                ])->getField('mark');                
            } else {
                $web_langdetect = (int)\think\Db::name('config')->where(['name'=>'web_langdetect'])->value('value');
                if (2 == $web_langdetect) {                    
                    $default_lang = get_http_accept_language();
                } else {  
                    $default_lang = \think\Db::name('language')->where([
                        'is_home_default'   => 1,
                    ])->getField('mark');  
                    ...
                    if($host){
                        $parts = explode('.', $host); 
                        $check =  \think\Db::name('language')->field('mark')->where(['url'=>$parts[0],'is_home_default'=> 0])->cache(true)->find();
                        if($check){
                            $default_lang = $check['mark'];
                        }
                    }                         
                }                
            }
...
if (!function_exists('get_current_lang')) 
{
     ...
     * 获取当前默认语言
     */
    function get_current_lang($is_force = false, $is_static = true)
    {
        if ($is_static) {
...
if (!function_exists('get_admin_lang')) 
{
     ...
     * 获取后台当前语言
     */
    function get_admin_lang($is_force = false, $is_static = true)
    {
        if ($is_static) {
        ...
        if (null === $admin_lang || $is_force) {
            ...
            if ($is_force || empty($admin_lang) || (!empty($admin_lang) && !preg_match('/^[a-z]{2}$/i', $admin_lang))) {
                $admin_lang = \think\Request::instance()->param('lang/s');
                if ($is_force || empty($admin_lang)) {
                    $admin_lang = \think\Db::name('language')->where([
                        'is_admin_default'   => 1,
                    ])->getField('mark');
                }
...
if (!function_exists('get_home_lang')) 
{
     ...
     * 获取前台当前语言
     */
    function get_home_lang($is_force = false, $is_static = true)
    {
        if ($is_static) {
        ...
        if (null === $home_lang || $is_force) {
            $keys = 'home_lang';
            $home_lang = \think\Request::instance()->param('lang/s');
            if ($is_force || empty($home_lang)){
                ...
                if ($is_force || empty($home_lang) || (!empty($home_lang) && !preg_match('/^[a-z]{2}$/i', $home_lang))) {
                    if ($is_force || empty($home_lang)) {
                        $web_langdetect = (int)\think\Db::name('config')->where(['name'=>'web_langdetect'])->value('value');
                        if (2 == $web_langdetect) {
                            $home_lang = get_http_accept_language();
                        } else {
...
if (!function_exists('switch_language')) 
{
    ...
    function switch_language() 
    {
        ...
        if (!empty($pathinfo)) {
            ...
            if (!empty($langList[$s_arr[0]])) {
                $current_lang = $s_arr[0];
            }
        }
        /*--end*/

        static $web_langdetect = null;
        null === $web_langdetect && $web_langdetect = (int)\think\Db::name('config')->where(['name'=>'web_langdetect'])->value('value');
        ...
        if (empty($current_lang)) {
            if ($is_admin) {
                ...
                if (2 == $web_langdetect) {
                    $current_lang = get_http_accept_language($langList);
                } else {
                    foreach ($langRow as $key => $val) {
                        if ($key == 0) {
                            $current_lang = $val['mark'];
        ...
        if (!empty($lang)) {   
            ...
            if($host){
                $parts = explode('.', $host); 
                $subdomain = $parts[0];
                $lres = $language_db->field('mark')->where(['url'=>trim($subdomain),'is_home_default'=>0])->cache(60)->find();
                if($lres){
                    $lang = $lres['mark'];
                }
            }                                
            // 处理访问不存在的语言
            $lang = empty($langList[$lang]) ? '' : $langList[$lang]['mark'];            
        }
        ...
        if (empty($lang)) {            
            ...
            if (!$is_admin) {
                $langsite_first_visit = cookie('langsite_first_visit');
                if (empty($langsite_first_visit)) {
                    if (!empty($web_langdetect)) {
                        cookie('langsite_first_visit', 1);
                        $http_accept_language = get_http_accept_language($langList);
                        if ($lang != $http_accept_language) {
                            if (2 == $web_langdetect) {
                                $url = ROOT_DIR.'/'; // 前台默认语言由浏览器当前语言标识决定，始终跳到首页
                            } else {
                                $url = langurl($langList[$http_accept_language]); // 跳到对应语言的首页
                            }
                            header('Location: '.$url);
                            exit;
                        }
            ...
            if (null === $web_basehost) { 
                $web_basehost = tpCache('web.web_basehost', [], $lang);
            }
            $web_subDomain = $request->domainPrefix('', true, $web_basehost);
            $subDomain = $request->domainPrefix();
            if (!empty($subDomain) && !in_array($subDomain, [$web_subDomain])) {
                $langInfo = $language_db->where('domain',$subDomain)->cache(true, EYOUCMS_CACHE_TIME, 'language')->find();
                if (!empty($langInfo['is_open'])) {
                    if (1 == $langInfo['is_home_default']) { // 设为默认语言跳转
        ...
        foreach ($langRow as $key => $val) {
            if ($val['mark'] == $lang) {
                ...
                $lang_info['lang_htmlmark'] = $lang_htmlmark;
                $lang_info['lang_mark'] = $val['mark'];
                $lang_info['lang_pageurl'] = $request->url(true);
                $lang_info['lang_url'] = langurl($val);
...
if (!function_exists('get_language_list')){
     ...
     * 获得语言站点的全部列表
     */
    function get_language_list($lang = '')
    {
        $result = \think\Cache::get('global_get_language_list');
        if (empty($result)) {
            $result = \think\Db::name('language')->field('*')
                ->where(['status'=>1])
                ->order("sort_order asc, id asc")
                ->getAllWithIndex('mark');
            \think\Cache::set('global_get_language_list', $result, null, 'language');
        }

        if (empty($lang)) {
            return $result;
        } else {
            return empty($result[$lang]) ? [] : $result[$lang];
        }
    }
}
    ...
    function get_archives_data($array = [], $id = '')
    {
        ...
        if (null === $array_new) {
            ...
            foreach ($archivesList as $key2 => $val2) {
                if (empty($arctypeList[$val2['typeid']])) {
                    $array_new[$val2['aid']] = $val2;
                } else {
    ...
    function getAllArctypeCount($home_lang,$info,$id = 0,$view_suffix = ".htm",$aid = 0)
    {
        ...
        foreach ($info as $k => $v) {
            ...
            if (!isset($tplData[$v['templist']])) {
                ...
                if ($content) {
                    preg_match_all('/\{zan:list(\s+)?(.*)\}/i', $content, $matchs);
                    if (!empty($matchs[0][0])) {
...
if (!function_exists('GetTotalArc')) 
{
    ...
    function GetTotalArc($typeid = 0)
    {
        if (empty($typeid)) {
            ...
            $count = false;//cache($cache_key);
            if (empty($count)) {
                if(false && version_compare(getCmsVersion(),'v1.6.6','>')) {
                    ...
                    $condition = [];
                    $allow_release_channel = config('global.allow_release_channel');
                    $allow_release_channel = !empty($allow_release_channel) ? implode(',', $allow_release_channel) : '';
                    if (!empty($allow_release_channel)) array_push($condition, "channel IN ($allow_release_channel)");
                    array_push($condition, "status = 1");
                    array_push($condition, "is_del = 0"); // 回收站功能
                    $typeid_arr = array_unique($typeids);
                    foreach ($typeid_arr as $_k => $_v) {
                        $stypeid_where .= " OR CONCAT(',', stypeid, ',') LIKE '%,{$_v},%' ";
                    }
                    $condition[] = Db::raw(" (typeid IN ({$typeid}) {$stypeid_where}) ");
                    $where_str = "";
                    ...
                    $count = \think\Db::name('archives')->where($where_str)->count('aid');
    ...
    function pay_success_logic($users_id = 0, $order_code = '', $pay_details = [], $paycode = 'alipay', $notify = true, $users = [], $config = [])
    {
        ...
        if (empty($orderData)) {
            ...
            if (false !== $ret) {
                ...
                $autoSendGoods = false;//$PayModel->afterVirtualProductPay($orderData, $config);
                ...
                // 异步处理
                /*if (true !== $notify || 'balance' == $paycode) {
                    // 订单支付通知
                    $params = [
                        'users_id' => $orderData['users_id'],
                ...
                // 订单操作完成，返回跳转
                $url = custom_url('user/Shop/shop_pay_success', ['order_id' => intval($orderData['order_id'])]);
                ...
                if (true === $autoSendGoods) {
                    $msg = lang('users292', [], get_current_lang());
                } else {
                    $msg = lang('users293', [], get_current_lang());
                }
                return [
                    'code'  => 1,
                    'msg'   => $msg,
                ...
                return [
                    'code'  => 0,
                    'msg'   => lang('users294', [], get_current_lang()),
                ];
            }
        }
            ...
            return [
                'code'  => 1,
                'msg'   => lang('users295', [], get_current_lang()),
            ];
        }
        else if (4 === intval($orderData['order_status'])) {
            return [
                'code'  => 0,
                'msg'   => lang('users296', [], get_current_lang()),
            ];
        }
        else {
            return [
                'code'  => 0,
                'msg'   => lang('users297', [], get_current_lang()),
            ];
        }
    }
}
...
if (!function_exists('clearHtmlCache')) {
    ...
    function clearHtmlCache($aids = [], $typeids = [])
    {
        ...
        if (!empty($aids)) {
            $htmlfilenameArr = [];
            $archivesList = \think\Db::name('archives')->field('aid,typeid,stypeid,htmlfilename')->where(['aid'=>['IN', $aids]])->select();
            foreach ($archivesList as $_k => $_v) {
                $htmlfilenameArr[] = md5($_v['htmlfilename']);
                $typeids[] = $_v['typeid'];
            ...
            if (!isset($seoData['seo_uphtml_after_pernext13']) || !empty($seoData['seo_uphtml_after_pernext13'])) {
                // 文档
                foreach ($htmlfilenameArr as $_k => $_v) {
...
if (!function_exists('sitemap_all')) 
{
     ...
     * 生成全部引擎sitemap
     */
    function sitemap_all($type = 'all', $filename = 'sitemap')
    {
        sitemap_xml($type, $filename);
    }
}
...
if (!function_exists('sitemap_xml')) 
{
     ...
     */
    function sitemap_xml($type = 'all', $filename = 'sitemap'){
        $globalConfig = tpCache('global');
        ...
        if (!empty($langRow['is_home_default'])) {
            $filename = ROOT_PATH . "{$filename}.xml";
            $filename_txt = ROOT_PATH . "{$filename}.txt";
        } else {
            $filename = ROOT_PATH . "{$filename}_{$lang}.xml";
            $filename_txt = ROOT_PATH . "{$filename}_{$lang}.txt";
        }
...
if (!function_exists('get_typeurl')) 
{
    ...
    function get_typeurl($arctype_info = array(), $admin = true,$domain_type = '')
    {
        ...
        if($default_lang<>$current_lang){
            $is_open = Db::name('language')->where(['mark'=>$current_lang,'is_open'=>1])->find();
            if($is_open){  
               $son_domain =  get_domain($current_lang); //二级域名          
            }
        }
        static $result = null;
        ...
        if (2 == $seo_pseudo && $admin) {
            ...
            $domain = !empty($son_domain)? false : true;
            $typeurl = typeurl("home/{$ctl_name}/lists", $arctype_info, true, $domain, $seo_pseudo, $seo_dynamic_format);
            if(!empty($son_domain)){                                             
                $typeurl = !empty($current_lang) ? str_replace('/'.$current_lang, '', $typeurl) : $typeurl;
                $typeurl = $son_domain.$typeurl;
            }
            // 自动隐藏index.php入口文件
            $typeurl = auto_hide_index($typeurl);
        }        
        // $typeurl = urldecode($typeurl);
...
if (!function_exists('get_arcurl_old')) 
{
    /**
     * 获取文档链接
     *
     ...
     */
    function get_arcurl_old($arcview_info = array(), $admin = true, $domain_type = '')
    {
        $domain = null; //static $domain = null;   //pc移动交替生成时候混乱
        null === $domain && $domain = request()->domain();
        if ($domain_type == 'mobile'){
        ...
        $seo_rewrite_view_format = config('ey_config.seo_rewrite_view_format'); //1父目录  3当前目录
        if ($admin) {
            if (2 == $seo_pseudo) {     
                static $lang = null;
                null === $lang && $lang = input('param.lang/s', 'cn');
                ...
                if (config('city_switch_on')) {
                    ...
                    if(in_array($seo_rewrite_format,[1,3])){ //目录模式
                        if(empty($arcview_info['stypeid'])){
                            $dirname = strtolower($ctl_name).'-details';
                        }else{                
                            if(strpos($arcview_info['stypeid'], ',') !== false) {
                                $tyarr = explode(',',$arcview_info['stypeid']);
                                $arcview_info['stypeid'] = $tyarr[0];
                            }            
                            $arctype = Db::name('arctype')->field('dirname,topid')->where(['lang'=>get_current_lang(),'id'=>$arcview_info['stypeid']])->find();
                            if($seo_rewrite_view_format==1){
                                if(empty($arctype['topid'])){
                                   $dirname = $arctype['dirname'];
                                }else{
                                   $dirname = Db::name('arctype')->where(['lang'=>get_current_lang(),'id'=>$arctype['topid']])->value('dirname');       
                                }        
                            }else{
                                $dirname = $arctype['dirname'];
                            }
                        }                                                                            
                        $arcurl = '/'.$dirname.'/'.$arcview_info['htmlfilename'].'.html';                    
                    }else{                                
            ...
            if (config('city_switch_on')) {
                ...
                if(in_array($seo_rewrite_format,[1,3])){ //目录模式
                    ...
                    $langinfo = Db::name('language')->field('is_home_default,url')->where(['mark'=>get_current_lang()])->find();
                    if($langinfo['is_home_default']){
                        $arcurl = '/'.$dirname.'/'.$arcview_info['htmlfilename'].'.html';  
                    }else{
                        if($langinfo['url']){
                            $arcurl = '/'.$dirname.'/'.$arcview_info['htmlfilename'].'.html';  
                        }else{
                            $arcurl = '/'.get_current_lang().'/'.$dirname.'/'.$arcview_info['htmlfilename'].'.html';  
                        }
                    }
                }else{                                
                  $arcurl = arcurl("home/{$ctl_name}/view", $arcview_info, true, $domain, $seo_pseudo, $seo_dynamic_format);
...
if (!function_exists('get_arcurl')) 
{
     ...
     * @param string  $domain_type   mobile：手机端
     */
    function get_arcurl($arcview_info = array(), $admin = true, $domain_type = '',$showlang='')
    {
        $domain = null; //static $domain = null;   //pc移动交替生成时候混乱
        null === $domain && $domain = request()->domain();
        if ($domain_type == 'mobile'){
        ...
        $seo_rewrite_view_format = config('ey_config.seo_rewrite_view_format'); //1父目录  3当前目录        
        $set_lang = '';
        $default_lang =  get_default_lang(); //默认
        $home_lang = get_current_lang(); //当前
        if(empty($showlang)){            
            $showlang = $default_lang<>$home_lang ?$home_lang:'';
        }else{ //后台切换语言
            $showlang = $home_lang <> $showlang ? $showlang:'';
        }            
        $son_domain = '';        
        if(!empty($showlang)){ //非默认语言
            $is_open = Db::name('language')->where(['mark'=>$showlang,'is_open'=>1])->find();
            if($is_open){  
                $son_domain =  get_domain($showlang); //二级域名          
            }else{                
                $set_lang = '/'.$showlang;
            }
        }
                ...
                if (config('city_switch_on')) {
                    ...
                    if(in_array($seo_rewrite_format,[1,3])){ //目录模式
                        if(empty($arcview_info['stypeid'])){
                            ...
                            $arctype = Db::name('arctype')->field('dirname,topid')->where(['lang'=>$home_lang,'id'=>$arcview_info['stypeid']])->find();
                            if($seo_rewrite_view_format==1){
                                if(empty($arctype['topid'])){
                                   $dirname = $arctype['dirname'];
                                }else{
                                   $dirname = Db::name('arctype')->where(['lang'=>$home_lang,'id'=>$arctype['topid']])->value('dirname');       
                                }        
                            }else{
                                $dirname = $arctype['dirname'];
                            }                         
                        }    
                        $son_domain = empty($son_domain)?get_domain():$son_domain;  
                        $arcurl = $son_domain.$set_lang.'/'.$dirname.'/'.$arcview_info['htmlfilename'].'.html';  
                    }else{
                        $domain = !empty($son_domain)? false : true;
                        $arcurl = arcurl("home/{$ctl_name}/view", $arcview_info, true, $domain, $seo_pseudo, $seo_dynamic_format);
                        if(!empty($son_domain)){                                             
                            $arcurl = !empty($showlang) ? str_replace('/'.$showlang, '', $arcurl) : $arcurl;
                            $arcurl = $son_domain.$arcurl;
                        }                    
                    } 
                }                
                // 自动隐藏index.php入口文件
...
if (!function_exists('handleEyouDataValidate')) 
{
     ...
     * $error     必填项不存在时的错误提示
     */
    function handleEyouDataValidate($required = '', $token = '', $post = [], $error = '')
    {
        $current_lang = get_current_lang();
        if (empty($required)) return lang('users128', [], $current_lang);
        if (empty($token)) return lang('users129', [], $current_lang);
        if (empty($post)) return lang('users130', [], $current_lang);
        if (empty($error)) $error = lang('users131', [], $current_lang);
        ...
        if (!$validate->batch()->check($post)) {
            ...
            $users132 = lang('users132', [], $current_lang);
            $users133 = lang('users133', [], $current_lang);
            $resultMsg = (empty($errorMsg[0]) || $errorMsg[0] == $users132) ? $users133 : $errorMsg[0];
        }
...
if (!function_exists('eyou_statistics_data')) {
    ...
    function eyou_statistics_data($type = 1,$change = 0,$now_time='',$action = 'inc')
    {
        try {
            ...
            $is_have = Db::name('statistics_data')->where(['date'=>$now_time,'type'=>$type])->find();
            if (!empty($is_have)){
                if ('inc' == $action){ //增加
                    //已经存在
                    if (in_array($type,[3,5])){
                        Db::name('statistics_data')->where(['date'=>$now_time,'type'=>$type])->setInc('total',$change);
                    }else{
                        Db::name('statistics_data')->where(['date'=>$now_time,'type'=>$type])->setInc('num',$change);
                    }
                }else{//减少
                    if (in_array($type,[3,5])){
                        Db::name('statistics_data')->where(['date'=>$now_time,'type'=>$type])->setDec('total',$change);
                    }else{
                        Db::name('statistics_data')->where(['date'=>$now_time,'type'=>$type])->setDec('num',$change);
                    }
                }
            }else{
                if ('inc' == $action){ //增加
...
if (!function_exists('SpLongBody')) 
{ 
    //自动截取方法
    function SpLongBody($mybody, $spsize)
    {
        // 新的字符截取逻辑 --- chenfy
        return handleFreeContent($mybody, $spsize);
        exit;
...
if (!function_exists('handleFreeContent')) 
{
    function handleFreeContent($content = '', $freeSize = 0, $encoding = 'utf-8')
    {
...
if (!function_exists('create_next_id')) 
{
    /**
     * 创建数据表的字段自增值
     ...
     * @return [type]        [description]
     */
    function create_next_id($table, $field, $where = [])
    {
        if (empty($where)) {
            $where = "1=1";
        }
        $value = (int)\think\Db::name($table)->where($where)->max($field);
        $value++;
...
if (!function_exists('get_cookie_langinfo')) 
{
    /**
     * 获取cookie里的语言信息
     * @return [type] [description]
     */
    function get_cookie_langinfo()
    {
        $lang_info = cookie('lang_info');
        $lang_info = empty($lang_info) ? [] : json_decode($lang_info, true);
        if (!empty($lang_info)) {
            $lang_info['lang_mark'] = preg_replace('/([^a-z]*)/i', '', $lang_info['lang_mark']);
            !empty($lang_info['lang_htmlmark']) && $lang_info['lang_htmlmark'] = preg_replace('/([^a-z]*)/i', '', $lang_info['lang_htmlmark']);
        }
...
if (!function_exists('get_currency_list'))
{
     ...
     * @return [type]       [description]
     */
    function get_currency_list($code = '')
    {
        static $currency_list = null;
        if (null === $currency_list) {
            $result = cache('global_get_currency_list');
            if (empty($result)) {
                $result = \think\Db::name('currency')->field('title,code,unit')
                    ->where(['status'=>1])
                    ->order("sort_order asc, id asc")
                    ->getAllWithIndex('code');
                cache('global_get_currency_list', $result, null, 'currency');
            }
            $currency_list = $result;
        }

        if (empty($code)) {
            return $currency_list;
        } else {
            return empty($currency_list[$code]) ? [] : $currency_list[$code];
        }
    }
}
...
if (!function_exists('get_currency_unit'))
{
     ...
     * @return [type]     [description]
     */
    function get_currency_unit($code = '')
    {
        $unit = '$';
        if (!empty($code)) {
            $currency_info = get_currency_list($code);
            $unit = empty($currency_info['unit']) ? $unit : $currency_info['unit'];
        }
...
if (!function_exists('check_template_uiset')) {
    /**
     * 检测模板是否支持可视化
     */
    function check_template_uiset($tempfile = '')
    {
        $is_uiset = false;
        if (empty($tempfile)) {
            $filelist = glob("./template/".TPL_THEME."pc/*.htm");
        } else {
        ...
        foreach ($filelist as $key => $val) {
            if (is_file($val)) {
                ...
                if (!empty($content) && preg_match('/{(\/)?zan\:ui([a-z]*)(\s+)/i', $content)) {
                    $is_uiset = true;
                    break;
                }
            }
...
if (!function_exists('get_http_accept_language'))
{
    /**
     * 获取浏览器的语言标识
     * 如果没有对应的网站语言，返回默认语言
     * @return [type] [description]
     */
    function get_http_accept_language($langList = [], $return_default_lang = true) {
        $http_accept_language = empty($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? '' : $_SERVER['HTTP_ACCEPT_LANGUAGE'];
        $http_accept_language = preg_replace('/^([\w\-]+)(,(.*))?$/i', '${1}', strtolower($http_accept_language));
        $http_accept_language = trim($http_accept_language);
        if (!empty($http_accept_language)) {
            if (in_array($http_accept_language, ['zh-cn','zh'])) {
            ...
            if (empty($langList)) {
                static $langRow = null;
                if (null === $langRow) {
                    $langRow = get_language_list();
                }
                $langList = $langRow;
            } else {
                if (!empty($langList[0])) {
                    $langList = convert_arr_key($langList, 'mark');
                }
            }
            if (empty($langList[$http_accept_language])) {
                if (empty($langList) || false === $return_default_lang) {
                    $http_accept_language = '';
                } else {
                    foreach ($langList as $key => $val) {
                        if (1 == $val['is_home_default']) {
                            $http_accept_language = $val['mark'];
                            break;
                        }
                    }
                }
