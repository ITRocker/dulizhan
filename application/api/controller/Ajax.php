...
//        $litpic_base64 = empty($post['litpic_base64']) ? '' : $post['litpic_base64'];
        ...
        if ($r !== false) {
            ...
            try {
                ...
                if (!empty($archivesInfo['articlePayInfo'])) {
                    ...
                    if ('add' == $opt) {
                        $archivesInfo['articlePayInfo']['lang'] = $this->home_lang;
    ...
    public function arcclick()
    {
        if (!IS_AJAX) {
            ...
            if (!empty($aids)) {
                ...
                $archives_db = Db::name('archives_'.$this->home_lang);
                if ('view' == $type) {
    ...
    public function downcount()
    {
        $aid = input('param.aid/d', 0);
        if (IS_AJAX && !empty($aid)) {
            $downcount = Db::name('archives_'.$this->home_lang)->where(array('aid' => $aid))->getField('downcount');
            echo($downcount);
            exit;
        } else {
            abort(404);
    ...
    public function collectnum()
    {
        $aid = input('param.aid/d', 0);
        if (IS_AJAX && !empty($aid)) {
            $collectnum = Db::name('users_collection')->where(['aid' => $aid, 'lang'=>$this->home_lang])->count();
            echo($collectnum);
            exit;
        } else {
            abort(404);
    ...
    public function arcpagelist()
    {
        ...
        $arcmultiRow = $arcmulti_db->where(['tagid' => $tagidmd5, 'lang'=>$this->home_lang])->find();
            ...
            if ('gbook_submit' == $type) {
                ...
                if (1 == $form_type) {
                    $info = Db::name('guestbook')->field('a.*, b.form_name')
                        ->alias('a')
                        ->join('form b',"a.typeid = b.form_id AND b.lang = '{$this->home_lang}'",'left')
                        ->where(['a.aid'=>$aid, 'a.form_type'=>$form_type])
                        ->find();
                } else {
                    $info = Db::name('guestbook')->field('a.*, b.typename as form_name')
                        ->alias('a')
                        ->join('arctype b',"a.typeid = b.id AND b.lang = '{$this->home_lang}'",'left')
                        ->where(['a.aid'=>$aid, 'a.form_type'=>$form_type])
                        ->find();
                }
                $city = "";
                $redata = get_ip2region($info['ip']);
                if (!empty($redata['code'])) {
                    $region_data = $redata['region_data'];
                    $country_str = empty($region_data[0]) ? '' : $region_data[0];
                    $province_str = empty($region_data[2]) ? '' : $region_data[2];
                    $city_str = empty($region_data[3]) ? '' : $region_data[3];
                    $city = $country_str.$province_str.$city_str;
                }
                $info['city'] = $city;

                // 判断标题拼接
                $web_name = tpCache('web.web_name', [], $this->home_lang);
                $web_name = $info['form_name'] . '-' . $web_name;

                // 拼装发送的字符串内容
                $attr_list = Db::name('guestbook_attribute')->where(['typeid'=>$tid,'form_type'=>$form_type,'lang'=>$this->home_lang])->order('attr_id asc')->select();
                $attr_values = Db::name('guestbook_attr')->field('attr_id,attr_value')->where(['aid'=>$aid,'form_type'=>$form_type,'lang'=>$this->home_lang])->getAllWithIndex('attr_id');
                ...
                $content .= '来源网址：' . $info['submit_url'] . '<br/>';
    ...
    public function SendMobileCode()
    {
        ...
        if (IS_AJAX_POST) {
            ...
            if (isset($post['scene']) && in_array($post['scene'], [7])) {
                ...
                // 图形验证码判断
                if (empty($post['code'])) $this->error(lang('users76', [], $this->home_lang));
            ...
            // 其他类型发送短信处理
                ...
                if (true === $post['is_mobile']) {
                    ...
                    $message = [
                        'mobile.require' => lang('users85', [], $this->home_lang),
                    ];
...
//                     }else
    ...
    public function collect_save()
    {
        ...
        if (IS_AJAX && !empty($aid)) {
            ...
            if (empty($row)) {
                $archivesInfo = Db::name('archives')->field('aid,title,litpic,channel,typeid')->where(['aid'=>$aid])->find();
                ...
                if (!empty($archivesInfo)) {
                    ...
                    if (!empty($r)) {
                        ...
                        $this->success(lang('users275', [], $this->home_lang), null, ['opt' => 'add']);
                    }
                }
            } else {
                ...
                $this->success(lang('users276', [], $this->home_lang), null, ['opt' => 'cancel']);
            }
            ...
            if (!$row) {
                ...
                if (!empty($r)) {
                    ...
                    Db::name('users_score')->add([
                        ...
                        'devote' => $scores_step,
                        'money' => 0.00,
                        'info' => lang('users69', [], $this->home_lang),
                        'lang' => $this->home_lang,
                        'add_time' => getTime(),
                        'update_time' => getTime(),
                        'current_score' => $users_scores,
                    ]);
                    $this->success(lang('users65', [], $this->home_lang), null, ['scores' => $users_scores,'score'=>$scores_step,'score_name'=>getUsersConfigData('score.score_name')]);
                }
                $this->error('未知错误', null);
            }
            $this->error(lang('users66', [], $this->home_lang), null);
        }
        abort(404);
    }
...
EOF;
    ...
    public function get_formreply_list()
    {
        ...
        $tagFormreply = new \think\template\taglib\zan\TagFormreply;
        ...
        if (!empty($attr_list) ) {
            ...
            /*拼接完整的formreply标签语法*/
            $innertext = "{zan:formreply typeid='{$typeid}' page='{$page}' pagesize='{$pagesize}' ordermode='{$ordermode}'}";
            $innertext .= $tpl_content;
            $innertext .= "{/zan:formreply}";
            /*--end*/
            $msg = $this->display($innertext); // 渲染模板标签语法
            $data['msg'] = $msg;
    ...
    public function cookieagrem_get_content()
    {
        $content = tpCache('cookieagrem.cookieagrem_content', [], $this->home_lang);
        $content = htmlspecialchars_decode($content);

        $this->success('ok', null, ['content'=>$content]);
    }

    public function cookieagrem_show_init()
    {
        \think\Session::pause(); // 暂停session，防止session阻塞机制
        $cookieagremData = tpCache('cookieagrem', [], $this->home_lang);
        $status = empty($cookieagremData['cookieagrem_status']) ? 0 : intval($cookieagremData['cookieagrem_status']);
        $position = empty($cookieagremData['cookieagrem_position']) ? 0 : intval($cookieagremData['cookieagrem_position']);
        if (empty($status) || session('?ey_cookie_httponly')) {
            $status = 0;
        }
        $this->success('ok', null, ['status'=>$status, 'position'=>$position]);
    }

    public function cookieagrem_session_set()
    {
        $name = input('param.name/s');
        $value = input('param.value/d', 0);
        if (in_array($name, ['ey_cookie_httponly'])) {
            session($name, $value);
        }
        $this->success('ok');
    }

    public function visit_log_ip()
    {
        $ip = getClientIP();// clientIP();
        if (!empty($ip)) {
            $a = date("Y-m-d");
            $b = strtotime($a, time());
            $c = $b + 86399;
            $where = [
                'ip' => trim($ip),
                'add_time' => ['between', [$b, $c]]
            ];
            $count = Db::name('visit_log_ip')->where($where)->count();
            if (!empty($count)) {
                $this->success('已存在记录');
            } else {
                $insert = [
                ...
                $result = Db::name('visit_log_ip')->insert($insert);
                if (!empty($result)) {
                    $where_ = [
                        'add_time' => ['between', [$b, $c]]
                    ];
                    $count_ = Db::name('visit_log')->where($where_)->count();
                    if (!empty($count_)) {
                        $result_ = Db::name('visit_log')->where($where_)->setInc('count', 1);
                    } else {
                        $insert_ = [
                            'dates' => trim($a),
                            'count' => 1,
                        ...
                        $result_ = Db::name('visit_log')->insert($insert_);
                    }
                }
                if (!empty($result_)) $this->success('新增记录成功');
            }
        }
        $this->error('新增记录失败');
    }
