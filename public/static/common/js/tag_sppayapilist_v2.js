...
function OrderPayPolling(showMsgCode) {
    ...
    var pay_id = json627847.pay_id ? json627847.pay_id : 0;
    var pay_mark = json627847.pay_mark ? json627847.pay_mark : 0;
    var pay_type = json627847.pay_type ? json627847.pay_type : 0;
    var unified_id = json627847.unified_id ? json627847.unified_id : 0;
    var unified_number = json627847.unified_number ? json627847.unified_number : 0;
    var transaction_type = json627847.transaction_type ? json627847.transaction_type : 0;
    if (!pay_id || !pay_mark || !unified_id || !unified_number || !transaction_type) {
        if (1 === parseInt(json627847.is_wap) && 'v2.x' == json627847.usersTpl2xVersion) {
...
// 外贸商城订单支付
function foreignTradeOrderPay(pay_id, pay_mark, unified_id, unified_number, transaction_type) {
    if (!pay_id || !pay_mark || !unified_id || !unified_number || !transaction_type) {
        showErrorAlert(ey_langpack_users337);
        return false;
    }
    json627847.pay_id = pay_id;
    json627847.pay_mark = pay_mark;
    json627847.unified_id = unified_id;
    json627847.unified_number = unified_number;
    json627847.transaction_type = transaction_type;

    if (1 === parseInt(json627847.is_wap) && 'v2.x' == json627847.usersTpl2xVersion) {
        showLayerLoad();
    } else {
        layer_loading(ey_langpack_users350);
    }
    ...
    $.ajax({
        ...
        url : json627847.SelectPayMethod,
        data: {
            pay_id: json627847.pay_id,
            pay_mark: json627847.pay_mark,
            unified_id: json627847.unified_id,
            unified_number: json627847.unified_number,
            transaction_type: json627847.transaction_type
        },
        dataType: 'json',
        success: function(res) {
            layer.closeAll();
            if (1 === parseInt(res.code)) {
                if (1 == res.data.is_paypal) {
                    if (1 === parseInt(json627847.is_wap) && 'v2.x' == json627847.usersTpl2xVersion) {
                        showLayerLoad();
                    } else {
                        layer_loading(ey_langpack_users351);
                    }
                    if (res.data.item_name && res.data.amount && res.data.invoice) {
                        $('#eyou_paypalForm, #eyou_itemName').val(res.data.item_name);
                        $('#eyou_paypalForm, #eyou_amount').val(res.data.amount);
                    ...
                    showErrorAlert('支付接口异常，请重新提交进行支付');
                }
            } else {
