...
function send_mobile_code() {
    ...
    if (!mobile) {
        ...
        layer.msg(ey_langpack_users85, {time: 1500});
        return false;
    }
    ...
    $.ajax({
        url: __root_dir__+'/index.php?m=api&c=Ajax&a=SendMobileCode&_ajax=1&lang='+__lang__,
        // source:2 登录
...
function mobile_fleshVerify(){
    var src =  __root_dir__+'/index.php?m=api&c=Ajax&a=vertify&type=users_mobile_login&lang='+__lang__;
    if (src.indexOf('?') > -1) {
        src += '&';
    } else {
        src += '?';
    ...
    $.ajax({
        async:false,
        url: __root_dir__+'/index.php?m=api&c=Ajax&a=get_token&name=__mobile_1_token__&lang='+__lang__,
        data: {_ajax:1},
        type:'GET',
        dataType:'html',
...
function checkMobileUserLogin1649732103() {
    ...
    layer_loading('正在处理');
    $.ajax({
        url:  __root_dir__+'/index.php?m=user&c=Users&a=mobile_login&lang='+__lang__,
