...
function ey_v378141(aid,cla,obj)
{
    ...
    ajax.open("post", root_dir_v379494+"/index.php?m=api&c=Ajax&a=collect_save&lang="+__lang__, true);
    // 给头部添加ajax信息
    ajax.setRequestHeader("X-Requested-With","XMLHttpRequest");
    // 如果需要像 HTML 表单那样 POST 数据，请使用 setRequestHeader() 来添加 HTTP 头。然后在 send() 方法中规定您希望发送的数据：
    ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    //步骤三:发送请求+数据
    ajax.send('aid='+aid+'&_ajax=1&lang='+lang_v379494);
            ...
            if (1 == res.code) {
                if ('on' == cla){
                    if (res.data.opt == 'add') {
                        ...
                        var classObj = document.getElementsByClassName("class_v378141_"+aid);
                        for (var i = 0; i < classObj.length; i++) {
                            classObj[i].innerHTML = collected_v379494;
                        }
                    } else {
                        if (collected_v379494) {
                        ...
                        var classObj = document.getElementsByClassName("class_v378141_"+aid);
                        for (var i = 0; i < classObj.length; i++) {
                            classObj[i].innerHTML = cancel_v379494;
                        }
                    }
                }else{
                    var afterHtml = '';
                    ...
                    var classObj = document.getElementsByClassName("class_v378141_"+aid);
                    for (var i = 0; i < classObj.length; i++) {
                        classObj[i].innerHTML = afterHtml;
                    }
                }
                if (!window.layer) {
                    alert(res.msg);
...
function ey_v377550(aid,cla)
{
    ...
    if ((document.getElementById("ey_v378141_"+aid) || document.getElementById("ey_cnum_v379494_"+aid)) && 0 < aid) {
        ...
        var ajax = new XMLHttpRequest();
        //步骤二:设置请求的url参数,参数一是请求的类型,参数二是请求的url,可以带参数,动态的传递参数starName到服务端
        ajax.open("get", root_dir_v379494+"/index.php?m=api&c=Ajax&a=get_collection&aid="+aid+"&lang="+lang_v379494, true);
        // 给头部添加ajax信息
        ajax.setRequestHeader("X-Requested-With","XMLHttpRequest");
        // 如果需要像 HTML 表单那样 POST 数据，请使用 setRequestHeader() 来添加 HTTP 头。然后在 send() 方法中规定您希望发送的数据：
        ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
