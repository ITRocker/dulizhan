...
function showSelectAddress(obj, type) {
    ...
    $.ajax({
        url : eyou_basefile + "?m=user&c=Shop&a=get_region_data&_ajax=1&lang="+__lang__,
...
function addressLoadHandle(loadContent, action, types) {
    if ('add' == action) {
        ...
        // 更新指定收货地址信息
        parent.$('#'+loadContent.addr_id+'_email').html(loadContent.email);
        parent.$('#'+loadContent.addr_id+'_mobile').html(loadContent.mobile);
        parent.$('#'+loadContent.addr_id+'_zipcode').html(loadContent.zipcode);
        ...
        parent.$('#'+loadContent.addr_id+'_info').html(loadContent.city_name +' '+ loadContent.province_name +' '+ loadContent.country_name);
    }
...
function handleParam(showMsg) {
    if (!$("#consignee").val()) {
        $("#consignee").focus();
        if (showMsg && 1 === parseInt(__isMobile__)) showLayerMsg(ey_langpack_users231);
        return false;
    }
    if (!$("#mobile").val()) {
        $("#mobile").focus();
        if (showMsg && 1 === parseInt(__isMobile__)) showLayerMsg(ey_langpack_users232);
        return false;
    }
    if ($("#province").val() == 0 || $("#city").val() == 0 || $("#district").val() == 0) {
        if (showMsg && 1 === parseInt(__isMobile__)) showLayerMsg(ey_langpack_users233);
        return false;
    }
    if (!$("#address").val()) {
        $("#address").focus();
        if (showMsg && 1 === parseInt(__isMobile__)) showLayerMsg(ey_langpack_users234);
        return false;
    }
    if (showMsg) {
        if (1 === parseInt(__isMobile__)) {
