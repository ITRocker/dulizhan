...
function showMbErrorAlert(msg) {
    layer.open({
        content: '<font color="red">'+ey_langpack_sys22+'：'+msg+'</font>', btn: ey_langpack_sys20
    });
}
...
function GetUploadify(num,elementid,path,callback,url,title,is_mobile)
{
    ...
    if (num > 0) {
        ...
        if (!is_mobile) is_mobile = __isMobile__;
        ...
        var area = 0 === parseInt(is_mobile) || 2 === parseInt(is_mobile) ? ['50%', '60%'] : ['100%', '100%'];
...
function layer_loading(msg){
    if (!msg || '正在处理' == msg) {
        if (typeof ey_langpack_sys26 === 'undefined') {
            ey_langpack_sys26 = "正在处理";
        }
        msg = ey_langpack_sys26;
    }
    if (typeof ey_langpack_sys27 === 'undefined') {
        ey_langpack_sys27 = "请勿刷新页面";
    }
...
function unifiedConfirmBox(msg, width, height, callback_1, btn, callback_2, callback_cancel) {
    ...
    if (typeof btn === 'undefined' || !btn) btn = [ey_langpack_sys20, ey_langpack_sys21];
...
function batchDelAction(obj, name, msg) {
    ...
    if (ids.length == 0) {
        showErrorAlert(ey_langpack_sys25);
        return false;
    }
    var data = {
        _ajax: 1,
