...
function ShopDelAddress(addr_id) {
    unifiedConfirmBox(ey_langpack_users222, '', '', function() {
        $.ajax({
...
function ShopPaymentPage() {
    layer_loading('<font id="loading_tips_230111">'+ey_langpack_sys26+'</font>');
    var timer = setTimeout(function() {
        $.ajax({
            ...
            success: function(res) {
                ...
                layer.closeAll();
                if (1 == res.code) {
                    foreignTradeOrderPay(res.data.pay_id, res.data.pay_mark, res.data.unified_id, res.data.unified_number, res.data.transaction_type);
                } else {
                    layer.closeAll();
                    if (res.data.field) {
                        $('#' + res.data.field).focus();
                        layer.msg(res.msg, {time: 1500});
                    }
                    else if (1 == res.data.add_addr) {
                        ShopAddAddress();
                    }
...
function selectAddr(obj) {
    var addr_id = $(obj).val();
    $.ajax({
        url : json612627.shop_order_addr,
        data: {addr_id: addr_id},
        type: 'post',
        dataType: 'json',
        success: function(res) {
            layer.closeAll();
            if (1 === parseInt(res.code)) {
                if (0 < parseInt($('#zan_shop_order_addr').length)) $('#zan_shop_order_addr').empty().html(res.data);
            } else {
                showErrorAlert(ey_langpack_sys18);
            }
        }
    });
}
