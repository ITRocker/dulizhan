...
// 加入购物车
function shop_add_cart(openPages) {
    ...
    if (parseInt(StockCountObj) == 0) {
        if (!window.layer) {
            alert(ey_langpack_users338);
        } else {
            layer.alert(ey_langpack_users338, {icon: 5, title: false, closeBtn: false});
        }
        return false;
    ...
    ajaxObj.onreadystatechange = function () {
        ...
        if (ajaxObj.readyState == 4 && ajaxObj.status == 200) {
            ...
            if (1 === parseInt(res.code)) {
                if (0 < parseInt(res.data.cartNum) || res.data.cartHtml) {
                    if (1 === parseInt(openPages)) {
                        updateCart(res.data.cartNum, res.data.cartHtml, true);
                    } else {
                        layer.msg(ey_langpack_users272);
                        updateCart(res.data.cartNum, res.data.cartHtml);
                    }
                } else {
                    // 是否要去购物车 
