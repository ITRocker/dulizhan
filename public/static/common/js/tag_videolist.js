...
function changeVideoUrl341922_sp1(id, aid, uhash, type) {
    ...
    ajax.open("post", vars1612143009.root_dir + "/index.php?m=home&c=View&a=pay_video_url&lang="+__lang__, true);
    // 给头部添加ajax信息
...
function checkAuth_v341922(fid, aid, type) {
    var url = vars1612143009.root_dir + "/index.php?m=home&c=View&a=play&aid="+aid+"&fid="+fid+"&lang="+__lang__;
    //步骤一:创建异步对象
    var ajax = new XMLHttpRequest();
    //步骤二:设置请求的url参数,参数一是请求的类型,参数二是请求的url,可以带参数,动态的传递参数starName到服务端
    ajax.open("post", vars1612143009.root_dir + "/index.php?m=home&c=View&a=check_auth&lang="+__lang__, true);
    // 给头部添加ajax信息
    ajax.setRequestHeader("X-Requested-With","XMLHttpRequest");
    // 如果需要像 HTML 表单那样 POST 数据，请使用 setRequestHeader() 来添加 HTTP 头。然后在 send() 方法中规定您希望发送的数据：
    ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function MediaOrderBuy_v341922(aid) {
    ...
    var ajax = new XMLHttpRequest();
    //步骤二:设置请求的url参数,参数一是请求的类型,参数二是请求的url,可以带参数,动态的传递参数starName到服务端
    ajax.open("post", vars1612143009.root_dir + "/index.php?m=user&c=Media&a=media_order_buy&lang="+__lang__, true);
    // 给头部添加ajax信息
    ajax.setRequestHeader("X-Requested-With","XMLHttpRequest");
    // 如果需要像 HTML 表单那样 POST 数据，请使用 setRequestHeader() 来添加 HTTP 头。然后在 send() 方法中规定您希望发送的数据：
    ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
